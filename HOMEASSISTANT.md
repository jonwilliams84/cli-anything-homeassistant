...
| `matter`       | **The Matter integration** (v1.56) — the nine websocket commands `components/matter/api.py` registers. `available` (no error when absent — scripts branch on it), `nodes` (device registry with `node_id` / `fabric_id` / `endpoint` decoded from the `("matter", "deviceid_<fabric:016X>-<node:016X>-…")` identifier), `commission` (QR / manual code, `--no-network-only` to move networks), `commission-on-network <PIN>` (`--ip` skips discovery), `set-wifi` / `set-thread` (credentials for LATER commissioning — moves nothing already on the network), `node-diagnostics` / `ping` / `interview` / `open-commissioning-window` / `remove-fabric` (confirmation-gated; all accept an entity id). Admin-only upstream: commission, set-wifi, set-thread, open-commissioning-window, remove-fabric; ping/interview/node-diagnostics are open to non-admin connections. A failed commissioning carries the Matter server's own error code; a `node_not_found` is re-raised as a ValueError that names `matter nodes` as the remedy. An instance without a Matter controller (or without the `python-matter-server` package a Core install needs) gets one clean sentence instead of `unknown_command` |

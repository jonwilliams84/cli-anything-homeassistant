...
def _validate_card_or_abort(
    card_or_dash, *, client=None, skip: bool = False, is_dashboard: bool = False
) -> None:
    ...
    try:
        if is_dashboard:
            ...
            installed = (
                lovelace_validate_core.installed_card_types(client) if client is not None else None
            )
    ...
    for w in warnings:
        click.echo(f"  [validate] WARN {w.get('path') or '<root>'}: {w.get('message')}", err=True)
    if errors:
...
def _config_diff(current, new, *, label_a: str = "live", label_b: str = "new") -> str:
    ...
    a_text = (
        json.dumps(current, indent=2, default=str, sort_keys=True) if current is not None else ""
    )
...
def config_test(ctx):
    ...
    emit(
        ctx, {"connected": True, "url": ctx.obj["url"], **(data if isinstance(data, dict) else {})}
    )
...
@click.option(
    "--grep", "-g", default=None, help="Only print lines matching this substring (case-insensitive)"
)
@click.option(
    "--regex", default=None, help="Only print lines matching this Python regex (case-insensitive)"
)
@click.option(
    "--exclude", "-x", multiple=True, help="Drop lines matching these substrings (repeatable)"
)
@click.option(
    "--summary",
    is_flag=True,
...
@click.option(
    "--component", default=None, help="Filter to a single component (e.g. 'custom_components.hon')."
)
@click.option(
    "--level",
    default=None,
...
@click.option("--top", default=None, type=int, help="Print the top N buckets instead of raw lines.")
@click.option(
    "--by",
    default="component",
...
def state_get(ctx, entity_id, attribute, state_only):
    ...
    if attribute is not None:
        ...
        if val is None and attribute not in full.get("attributes", {}):
            click.echo(f"error: entity {entity_id!r} has no attribute {attribute!r}", err=True)
            sys.exit(1)
        if ctx.obj.get("as_json"):
            emit(ctx, val)
        else:
...
@click.option(
    "--attr", "-a", multiple=True, help="Attribute key=value (repeatable, JSON-decoded values)"
)
@click.pass_context
...
def service_describe(ctx, domain_arg, service_arg):
    ...
    if info is None:
        click.echo(f"error: {domain_arg}.{service_arg} not in service registry", err=True)
        sys.exit(1)
    emit(ctx, info)
...
@click.option(
    "--data", "-D", multiple=True, help="Event data key=value (JSON values supported, repeatable)"
)
...
@click.option(
    "--var", "-V", multiple=True, help="Template variable key=value (repeatable, JSON values)"
)
@click.pass_context
...
@click.pass_context
def area_update(
    ctx, area_id, name, floor_id, icon, picture, aliases, labels, clear_aliases, clear_labels
):
...
@click.argument("name")
@click.option("--level", type=int, default=None, help="Storey index (lower=lower; 0=ground)")
@click.option("--icon", default=None)
@click.option("--alias", "aliases", multiple=True)
@click.pass_context
...
@click.option("--label", "labels", multiple=True, help="Replace the label list (repeatable)")
@click.option("--alias", "aliases", multiple=True)
@click.option("--clear-labels", is_flag=True, default=False)
@click.option("--clear-aliases", is_flag=True, default=False)
...
@click.option("--pattern", default=None, help="Regex matched against entity_id")
@click.option("--domain", default=None, help="Restrict to one domain")
@click.option("--area", "area_id", default=None, help="Restrict to entities already in this area")
@click.option(
    "--current-label", default=None, help="Restrict to entities that already have this label"
)
@click.option(
    "--integration", default=None, help="Restrict to one platform/integration (entity.platform)"
)
@click.option("--set-name", default=None, help="Set the friendly_name on every match")
@click.option("--set-area", default=None, help="Move every match to this area")
@click.option("--add-label", "add_labels", multiple=True, help="Append labels (repeatable)")
@click.option(
    "--set-labels", "set_labels", multiple=True, help="Replace the label list on every match"
)
@click.option(
    "--remove-label", "remove_labels", multiple=True, help="Remove specific labels (repeatable)"
)
@click.option(
    "--rename-prefix",
    default=None,
...
@click.option("--dry-run", is_flag=True, default=False)
@click.option(
    "--limit", type=int, default=None, help="Only touch the first N matches (useful with --dry-run)"
)
@click.pass_context
def entity_bulk_update(
    ...
    for e in matches:
        ...
        if set_name is not None:
            row["name"] = set_name
        if set_area is not None:
            row["area_id"] = set_area
    ...
    summary = registry_core.bulk_update_entities(client, updates=updates, dry_run=dry_run)
    emit(
        ctx,
        {
            "matched": len(matches),
...
@click.argument("device_id")
@click.option(
    "--name", "name_by_user", default=None, help="User-set name (overrides manufacturer's default)"
)
@click.option("--area", "area_id", default=None)
@click.option("--label", "labels", multiple=True, help="Replace the label list (repeatable)")
@click.option("--clear-labels", is_flag=True, default=False)
...
@click.option("--enable", is_flag=True, default=False)
@click.pass_context
def device_update(ctx, device_id, name_by_user, area_id, labels, clear_labels, disable, enable):
    if disable and enable:
        _abort("--disable and --enable are mutually exclusive")
    kwargs: dict = {}
    if name_by_user is not None:
        kwargs["name_by_user"] = name_by_user
    if area_id is not None:
        kwargs["area_id"] = area_id
    if clear_labels:
        kwargs["labels"] = []
...
@click.argument("name")
@click.option("--user", "user_id", default=None, help="Link to this HA user_id")
...
def person_create(ctx, name, user_id, device_trackers, picture):
    emit(
        ...
        persons_core.create(
            ...
            name=name,
            user_id=user_id,
            device_trackers=list(device_trackers) or None,
            picture=picture,
        ),
    )
...
@click.option("--picture", default=None)
@click.pass_context
def person_update(ctx, person_id, name, user_id, device_trackers, clear_device_trackers, picture):
...
def tag_create(ctx, tag_id, name, description):
    ...
    emit(ctx, tags_core.create(make_client(ctx), tag_id, name=name, description=description))
...
def tag_update(ctx, tag_id, name, description):
    emit(ctx, tags_core.update(make_client(ctx), tag_id, name=name, description=description))
...
@click.option(
    ...
    type=int,
    default=None,
    help="Return only the most-recent N traces. Default: all (HA keeps up to 5 per automation).",
)
@click.option(
    "--since",
    default=None,
...
def automation_traces(ctx, entity_id, limit, since):
    ...
    if since:
        ...
        traces = [t for t in traces if _trace_start(t) and _trace_start(t) >= cutoff]
...
def automation_trace(ctx, entity_id, run_id_arg, run_id_opt, summary, show_vars):
    ...
    out = {
        ...
        "trigger": (
            full.get("trace", {}).get("trigger", [{}])[0].get("result", {}).get("description")
            if full.get("trace", {}).get("trigger")
            else None
        ),
...
@click.option(
    ...
    "run_id_opt",
    default=None,
    help="Specific run id (default: most recent). May also be given positionally after ENTITY_ID.",
)
@click.option(
    "--vars",
    "show_vars",
...
@cli.command("history")
@click.option(
    "--entity", "-e", "entity_ids", multiple=True, help="Filter to entity_id (repeatable)"
)
@click.option(
    "--hours",
    default=None,
...
@click.option("--end", "end_iso", default=None, help="Explicit ISO-8601 end time")
@click.option("--minimal/--full", "minimal", default=True)
@click.option(
    "--no-attributes",
    is_flag=True,
    default=False,
    help="Omit per-sample attributes blob. Big speed/size win on installs with heavy entities.",
)
@click.option(
...
@click.pass_context
def history_cmd(
    ctx, entity_ids, hours, start_iso, end_iso, minimal, no_attributes, significant_changes_only
):
...
@click.option(
    "--hours", default=24.0, type=float, help="Window in hours to count history points (default 24)"
)
@click.pass_context
...
def recorder_find_unrecorded(ctx, entity_ids, hours):
    ...
    emit(ctx, recorder_core.find_unrecorded(make_client(ctx), entity_ids, hours=hours))
...
@click.option(
    ...
    "template_type",
    default="sensor",
    type=click.Choice(["sensor", "binary_sensor", "number", "switch", "select", "button"]),
    help="Helper type (default: sensor)",
)
@click.option(
    "--unit", "unit_of_measurement", default=None, help="Unit of measurement (e.g. Mbps, %)"
)
@click.option("--device-class", default=None)
@click.option("--state-class", default=None, help="e.g. measurement, total, total_increasing")
@click.option(
    "--extra-file",
    type=click.Path(exists=True, dir_okay=False),
...
@click.option(
    "--max", "max_hits", default=30, type=int, help="Cap per-kind result count (default 30)."
)
@click.pass_context
def entity_references_cmd(ctx, entity_id, kind, max_hits):
    ...
    emit(
        ctx,
        references_core.find_references(
            make_client(ctx),
            entity_id,
...
@click.argument("level", type=click.Choice(sorted(logger_core.VALID_LEVELS), case_sensitive=False))
@click.pass_context
...
@click.option(
    "--prefix", default="homeassistant", help="Discovery topic prefix (default 'homeassistant')"
)
@click.option(
    "--timeout", default=5.0, type=float, help="How long to collect retained messages (default 5s)"
)
@click.pass_context
def mqtt_discovery_list(ctx, prefix, timeout):
    ...
    emit(
        ctx,
        mqtt_discovery_core.list_discovered(
            make_client(ctx),
            prefix=prefix,
            timeout=timeout,
        ),
...
@click.option("--filename", default=None, help="Required for --mode yaml; path under /config")
@click.option("--show-in-sidebar/--no-show-in-sidebar", default=True)
@click.option("--require-admin/--no-require-admin", default=False)
@click.pass_context
def lovelace_dashboards_create(
    ctx, url_path, title, mode, icon, filename, show_in_sidebar, require_admin
):
    ...
    emit(
        ctx,
        lovelace_core.create_dashboard(
            make_client(ctx),
            url_path,
            title,
            mode=mode,
...
@click.option("--icon", default=None)
@click.option(
    "--url-path", default=None, help="Change the URL path under which the dashboard is served"
)
@click.option("--show-in-sidebar/--no-show-in-sidebar", "show_in_sidebar", default=None)
@click.option("--require-admin/--no-require-admin", "require_admin", default=None)
@click.pass_context
def lovelace_dashboards_update(
    ctx, dashboard_id, title, icon, url_path, show_in_sidebar, require_admin
):
    ...
    emit(
        ctx,
        lovelace_core.update_dashboard(
            make_client(ctx),
            dashboard_id,
            title=title,
            icon=icon,
...
@click.option(
    "--out", "-o", default=None, type=click.Path(), help="Write JSON to this file (default: stdout)"
)
@click.pass_context
    ...
    if dry_run:
        ...
        emit(
            ctx,
            {
                ...
                "diff": _config_diff(
                    current, cfg, label_a=f"live:{url_path}", label_b=f"new:{config_file}"
                ),
            },
        )
        return
...
@click.argument("url_path")
@click.argument("view_file", type=click.Path(exists=True, dir_okay=False))
@click.option("--index", type=int, default=None, help="Insert at this index (default: append)")
@click.option("--dry-run", is_flag=True, default=False)
@click.option(
    "--no-validate",
    is_flag=True,
...
def lovelace_resources_delete(ctx, resource_id):
    ...
    emit(
        ...
        {
            "deleted": resource_id,
            "result": lovelace_core.delete_resource(make_client(ctx), resource_id),
        },
    )
...
@click.argument("url")
@click.option("--type", "res_type", default="module", type=click.Choice(["module", "css", "js"]))
@click.pass_context
...
@click.option(
    "--contains", default=None, help="Match cards whose serialised JSON contains this substring"
)
@click.pass_context
...
@click.option(
    "--dry-run", is_flag=True, default=False, help="Show what would change without saving"
)
@click.option(
    "--no-validate",
    is_flag=True,
...
def lovelace_card_delete(ctx, url_path, pointer, yes, dry_run):
    ...
    try:
        ...
        sys.exit(1)
    if dry_run:
        emit(ctx, {"dry_run": True, "would_delete": pointer, "type": old_card.get("type")})
        return
    if not yes and not click.confirm(
        f"Delete {old_card.get('type')!r} card at {pointer}?",
        default=False,
    ):
        click.echo("aborted")
        return
...
@click.argument("card_file", type=click.Path(exists=True, dir_okay=False))
@click.option(
    "--position", default=None, type=int, help="Position in the parent's cards[] (default: append)"
)
@click.option(
    "--dry-run",
    is_flag=True,
...
def lovelace_card_insert(ctx, url_path, parent_pointer, card_file, position, dry_run, no_validate):
    ...
    if dry_run:
        emit(
            ...
            {
                "dry_run": True,
                "would_insert_into": parent_pointer,
        ...
        return
    lovelace_core.save_dashboard_config(client, url_path, cfg)
    emit(ctx, {"inserted_into": parent_pointer, "type": new_card.get("type"), "position": position})
...
def lovelace_lint(ctx, url_path, check_types, check_templates):
    ...
    if check_types and known_types is not None:
        ...
        result["unknown_card_types"] = [
            x
            for x in result["unknown_card_types"]
...
@click.argument("url_path")
@click.option(
    "--type", "types", multiple=True, help="Drop any card whose type matches (repeatable)"
)
@click.option(
    "--entity-prefix",
    "entity_prefixes",
...
@click.option("--dry-run", is_flag=True, default=False)
@click.pass_context
def lovelace_prune(ctx, url_path, types, entity_prefixes, markdown_contains, subheadings, dry_run):
    ...
    if not (types or entity_prefixes or markdown_contains or subheadings):
        click.echo(
            "error: pass at least one filter "
...
def lovelace_mirror(
    ...
    emit(
        ...
        lovelace_mirror_core.mirror(
            ...
            blocked_entity_prefixes=set(blocked_entity_prefixes)
            if blocked_entity_prefixes
            else None,
            blocked_markdown_contains=set(blocked_markdown_contains)
            if blocked_markdown_contains
            else None,
            dry_run=dry_run,
        ),
    )
...
@click.argument("url_path")
@click.argument("pointer")
@click.option("--value", default=None, help="JSON value to set at the pointer (use '-' for stdin)")
@click.option(
    "--value-file", type=click.Path(exists=True, dir_okay=False), help="Read JSON value from a file"
)
@click.option(
    "--delete",
    is_flag=True,
...
@click.option(
    "--dry-run", is_flag=True, default=False, help="Print the resulting config but don't save it"
)
@click.pass_context
def lovelace_patch(ctx, url_path, pointer, value, value_file, delete, dry_run):
    ...
    if delete and (value or value_file):
        click.echo("error: --delete is mutually exclusive with --value/--value-file", err=True)
        sys.exit(1)
    if not delete:
        if value_file:
            ...
            click.echo("error: provide --value, --value-file, or --delete", err=True)
            sys.exit(1)
    try:
        if delete:
        ...
        sys.exit(1)
    if dry_run:
        emit(ctx, {"dry_run": True, "pointer": pointer, "would_save_to": url_path})
        return
    lovelace_core.save_dashboard_config(client, url_path, cfg)
    emit(ctx, {"patched": pointer, "deleted": delete, "url_path": url_path})
...
def config_entry_delete(ctx, entry_id, yes):
    ...
    emit(
        ...
        {
            "deleted": entry_id,
            "result": config_entries_core.delete_entry(make_client(ctx), entry_id),
        },
    )
...
def config_entry_reload(ctx, entry_id):
    ...
    emit(
        ...
        {
            "reloaded": entry_id,
            "result": config_entries_core.reload_entry(make_client(ctx), entry_id),
        },
    )
...
@click.option("--data", "-D", multiple=True, help="key=value (JSON values supported, repeatable)")
@click.option(
    "--data-file",
    type=click.Path(exists=True, dir_okay=False),
...
def config_entry_options_configure(ctx, flow_id, data, data_file):
    ...
    emit(ctx, config_entries_core.options_flow_configure(make_client(ctx), flow_id, user_input))
...
def config_entry_options_set(ctx, entry_id, data, data_file):
    ...
    if not user_input:
        ...
        sys.exit(1)
    emit(ctx, config_entries_core.options_flow_set(make_client(ctx), entry_id, user_input))
...
@click.option(
    "--data", "-D", multiple=True, help="entry.data key=value (JSON values supported, repeatable)"
)
@click.pass_context
def config_entry_update(ctx, entry_id, title, data):
    ...
    emit(
        ctx,
        config_entries_core.update_entry(make_client(ctx), entry_id, title=title, options=options),
    )
...
def automation_save(ctx, entity_id, config_file, dry_run, yes):
    ...
    if dry_run:
        try:
            ...
            current = None
        diff = _config_diff(current, cfg, label_a=f"live:{entity_id}", label_b=f"new:{config_file}")
    ...
    emit(
        ...
        {
            "saved": entity_id,
            "result": automation_core.save_config(make_client(ctx), entity_id, cfg),
        },
    )
...
def script_save(ctx, entity_id, config_file, dry_run, yes):
    ...
    emit(
        ctx,
        {"saved": entity_id, "result": script_core.save_config(make_client(ctx), entity_id, cfg)},
    )
...
def mqtt_publish(ctx, topic, payload, qos, retain, from_file):
    ...
    emit(
        ctx,
        {"published": True, "topic": topic, "qos": qos, "retain": retain, "bytes": len(payload)},
    )
...
def mqtt_subscribe(ctx, topic, limit, timeout, qos, out):
    ...
    try:
        client.ws_subscribe("mqtt/subscribe", {"topic": topic, "qos": qos}, on_msg, stop)
    except KeyboardInterrupt:
        stop.set()
    finally:
        if out_fh is not None:
    ...
    if ctx.obj.get("as_json"):
        emit(
            ctx,
            seen if buffer_messages else {"received": count[0], "out": out, "streamed_only": True},
        )
    else:
...
def helpers_input_select_sync(ctx, src_entity_id, dst_entity_id, fallback):
    ...
    emit(
        ctx,
        helpers_core.input_select_sync(
...
def helpers_input_select_create(ctx, name, options, icon, initial):
    ...
    emit(
        ctx,
        helpers_core.input_select_create(
            make_client(ctx),
            name=name,
...
def helpers_input_select_update(ctx, entity_id, options, from_file, name, icon, initial):
    ...
    emit(
        ctx,
        helpers_core.input_select_update(
            make_client(ctx),
            entity_id,
            options=opts,
            name=name,
...
def helpers_input_boolean_create(ctx, name, icon, initial):
    ...
    emit(
        ctx,
        helpers_core.input_boolean_create(make_client(ctx), name=name, icon=icon, initial=initial),
    )
...
def helpers_input_boolean_update(ctx, entity_id, name, icon, initial):
    ...
    emit(
        ctx,
        helpers_core.input_boolean_update(
...
def helpers_input_button_update(ctx, entity_id, name, icon):
    emit(ctx, helpers_core.input_button_update(make_client(ctx), entity_id, name=name, icon=icon))
...
@click.pass_context
def helpers_input_number_create(
    ctx, name, min_, max_, step, mode, unit_of_measurement, icon, initial
):
    emit(
        ctx,
        helpers_core.input_number_create(
            make_client(ctx),
            name=name,
            min=min_,
            max=max_,
...
@click.option("--initial", type=float, default=None)
@click.pass_context
def helpers_input_number_update(
    ctx, entity_id, name, min_, max_, step, mode, unit_of_measurement, icon, initial
):
    emit(
        ctx,
        helpers_core.input_number_update(
            make_client(ctx),
            entity_id,
            name=name,
            min=min_,
...
def helpers_input_text_create(ctx, name, min_, max_, pattern, mode, icon, initial):
    emit(
        ctx,
        helpers_core.input_text_create(
            make_client(ctx),
            name=name,
            min=min_,
            max=max_,
...
@click.option("--initial", default=None)
@click.pass_context
def helpers_input_text_update(ctx, entity_id, name, min_, max_, pattern, mode, icon, initial):
    emit(
        ctx,
        helpers_core.input_text_update(
            make_client(ctx),
            entity_id,
            name=name,
            min=min_,
...
@click.option("--icon", default=None)
@click.option(
    "--initial", default=None, help="Initial value: YYYY-MM-DD, HH:MM:SS, or YYYY-MM-DD HH:MM:SS"
)
@click.pass_context
def helpers_input_datetime_create(ctx, name, has_date, has_time, icon, initial):
    emit(
        ctx,
        helpers_core.input_datetime_create(
            make_client(ctx),
            name=name,
            has_date=has_date,
            has_time=has_time,
...
@click.option("--initial", default=None)
@click.pass_context
def helpers_input_datetime_update(ctx, entity_id, name, has_date, has_time, icon, initial):
    emit(
        ctx,
        helpers_core.input_datetime_update(
            make_client(ctx),
            entity_id,
            name=name,
            has_date=has_date,
...
def helpers_input_datetime_set(ctx, entity_id, date, time, datetime_):
    emit(
        ctx,
        helpers_core.input_datetime_set(
...
@click.option("--restore/--no-restore", default=True, help="Keep value across HA restart")
@click.option("--icon", default=None)
@click.pass_context
def helpers_counter_create(ctx, name, initial, step, minimum, maximum, restore, icon):
    emit(
        ctx,
        helpers_core.counter_create(
            make_client(ctx),
            name=name,
            initial=initial,
            step=step,
...
@click.option("--icon", default=None)
@click.pass_context
def helpers_counter_update(ctx, entity_id, name, initial, step, minimum, maximum, restore, icon):
    emit(
        ctx,
        helpers_core.counter_update(
            make_client(ctx),
            entity_id,
            name=name,
            initial=initial,
...
def helpers_counter_delete(ctx, entity_id):
    emit(
        ctx,
        {"deleted": entity_id, "result": helpers_core.counter_delete(make_client(ctx), entity_id)},
    )
...
def helpers_timer_create(ctx, name, duration, restore, icon):
    emit(
        ctx,
        helpers_core.timer_create(
            make_client(ctx), name=name, duration=duration, restore=restore, icon=icon
        ),
    )
...
def helpers_timer_update(ctx, entity_id, name, duration, restore, icon):
    emit(
        ctx,
        helpers_core.timer_update(
            make_client(ctx), entity_id, name=name, duration=duration, restore=restore, icon=icon
        ),
    )
...
def helpers_timer_delete(ctx, entity_id):
    emit(
        ctx,
        {"deleted": entity_id, "result": helpers_core.timer_delete(make_client(ctx), entity_id)},
    )
...
def helpers_schedule_delete(ctx, entity_id):
    emit(
        ctx,
        {"deleted": entity_id, "result": helpers_core.schedule_delete(make_client(ctx), entity_id)},
    )
...
def helpers_schedule_update(ctx, entity_id, from_file, name):
    ...
    if not isinstance(cfg, dict):
        click.echo("error: --from-file must contain a JSON object", err=True)
        sys.exit(1)
    emit(ctx, helpers_core.schedule_update(make_client(ctx), entity_id, name=name, **cfg))
...
@click.option(
    "--url-path", "-u", multiple=True, help="One or more dashboard url_paths (default: all)"
)
@click.pass_context
def lovelace_tools_backup(ctx, out_dir, url_path):
    ...
    for path in targets:
        ...
        written.append({"url_path": path, "file": out, "views": len(cfg.get("views", []))})
    emit(ctx, written)
...
@click.option(
    "--summary/--full", default=True, help="Summary by default; --full for the unified JSON diff"
)
@click.pass_context
def lovelace_tools_diff(ctx, file_a, file_b, summary):
    ...
    if summary:
        ...
        diff = "".join(difflib.unified_diff(a_text, b_text, fromfile=file_a, tofile=file_b, n=3))
        if ctx.obj.get("as_json"):
            emit(ctx, {"diff": diff})
        else:
        ...
        rows.append(
            {
                ...
                "type": b.get("type"),
                "agents": b.get("agent_ids") or list((b.get("agents") or {}).keys()) or None,
            }
        )
    emit(ctx, rows)
...
@click.option(
    "--password", default=None, help="Encrypt with this password (writes/reads must match)"
)
@click.option(
    "--no-database",
    is_flag=True,
...
@click.argument("backup_id")
@click.option(
    "--agent", "agent_ids", multiple=True, help="Delete only from these agents (default: all)"
)
...
@click.argument("backup_id")
@click.option("--password", default=None, help="Password for an encrypted backup")
@click.option("--no-database", is_flag=True, default=False)
@click.option(
    "--folder", "restore_folders", multiple=True, help="Folders to restore (repeatable; HA 2024.7+)"
)
@click.option("--addon", "restore_addons", multiple=True, help="Add-ons to restore (repeatable)")
@click.option("--agent", "agent_id", default=None, help="Restore from this specific agent")
@click.confirmation_option(prompt="Restore this backup? HA will restart.")
@click.pass_context
def backup_restore(
    ctx, backup_id, password, no_database, restore_folders, restore_addons, agent_id
):
    emit(
        ctx,
        backup_core.restore(
...
def system_restart(ctx, safe_mode):
    ...
    emit(
        ctx,
        {"restart": "sent", "result": control_core.restart(make_client(ctx), safe_mode=safe_mode)},
    )
...
@click.pass_context
def system_stop(ctx):
    emit(ctx, {"stop": "sent", "result": control_core.stop(make_client(ctx))})
...
def system_reload_core(ctx):
    emit(
        ctx,
        {"reloaded": "core_config", "result": control_core.reload_core_config(make_client(ctx))},
    )
...
@click.option("--severity", default=None, type=click.Choice(["error", "warning", "critical"]))
...
@click.argument("issue_id")
@click.option("--domain", required=True, help="The issue's domain (run `repairs list` to see)")
@click.option(
    "--undismiss", is_flag=True, default=False, help="Un-dismiss an issue you previously ignored"
)
@click.pass_context
...
@click.option("--title", default=None)
@click.option(
    "--id", "notification_id", default=None, help="Stable id — calls with same id update in place"
)
@click.pass_context
def notifications_create(ctx, message, title, notification_id):
    emit(
        ctx,
        notifications_core.create(
            make_client(ctx),
            message=message,
            title=title,
...
@click.argument("event_type_pos", required=False, default=None, metavar="[EVENT_TYPE]")
@click.option(
    "--type",
    "event_type_opt",
    default=None,
...
@click.option("--duration", type=float, default=10.0, help="Seconds to listen (default 10)")
...
def event_subscribe(ctx, event_type_pos, event_type_opt, duration, limit, filters):
    ...
    for raw in filters:
        if "=" not in raw:
            raise click.ClickException(f"--filter expects key=value, got {raw!r}")
        k, v = raw.split("=", 1)
...
@state.command("watch")
@click.argument("entity_id")
@click.option("--until-state", default=None, help="Stop as soon as the entity reaches this state")
...
@click.argument("entry_id")
@click.option(
    "-o", "--out", type=click.Path(), default=None, help="Save to file instead of printing"
)
@click.pass_context
...
@click.option("--grep", default=None, help="Substring filter on statistic_id (case-insensitive)")
@click.pass_context
...
def statistics_metadata(ctx, statistic_ids):
    ...
    emit(
        ctx,
        statistics_core.get_metadata(
            make_client(ctx),
            list(statistic_ids) or None,
        ),
    )
...
@click.argument("statistic_ids", nargs=-1)
@click.option(
    "--period", default="hour", type=click.Choice(["5minute", "hour", "day", "week", "month"])
)
@click.option("--start", "start_time", default=None, help="ISO timestamp (default: 24h ago, UTC)")
...
@click.pass_context
def statistics_series(ctx, statistic_ids, period, start_time, end_time, types, unit_pairs):
...
def statistics_update_metadata(ctx, statistic_id, unit_of_measurement):
    ...
    emit(
        ctx,
        statistics_core.update_metadata(
            make_client(ctx),
...
@assist.command("ask")
@click.argument("text")
@click.option("--conversation-id", default=None, help="Reuse a previous conversation context")
@click.option("--language", default=None)
@click.option(
    "--agent", "agent_id", default=None, help="Specific Assist pipeline or conversation agent id"
)
@click.pass_context
...
def updates_list(ctx, include_off):
    emit(
        ctx,
        updates_core.list_updates(
            make_client(ctx),
            available_only=not include_off,
        ),
    )
...
@click.option("--backup", is_flag=True, default=False, help="Backup before installing")
@click.pass_context
def updates_install(ctx, entity_id, version, backup):
    emit(
        ctx,
        updates_core.install(
            make_client(ctx),
            entity_id,
            version=version,
            backup=backup,
...
@click.argument("entity_id")
@click.option(
    "--history/--no-history", default=False, help="Include recent history (slower on busy entities)"
)
...
@click.option(
    "--ref-kind",
    multiple=True,
    type=click.Choice(["automation", "script", "scene", "template_helper", "lovelace"]),
    help="Restrict references search to these kinds",
)
@click.pass_context
def entity_inspect(ctx, entity_id, history, history_hours, no_references, ref_kind):
    """One-shot diagnostic: state + registry + device + area + history + references."""
    emit(
        ctx,
        inspect_core.inspect_entity(
            make_client(ctx),
            entity_id,
...
@click.argument("flow_id")
@click.option(
    "--data", "-D", multiple=True, help="Form field key=value (repeatable, JSON values supported)"
)
@click.option(
    "--data-file",
    type=click.Path(exists=True, dir_okay=False),
...
@click.option("--source-url", default=None, help="Original import URL (preserved for traceability)")
@click.pass_context
...
@click.option(
    "--co2-signal", required=True, help="The sensor.* entity from the co2signal integration"
)
@click.option("--start", "start_time", required=True, help="ISO-8601 start time")
@click.option("--end", "end_time", default=None)
@click.option(
    "--period", default="hour", type=click.Choice(["5minute", "hour", "day", "week", "month"])
)
@click.pass_context
def energy_fossil(ctx, energy_statistic_ids, co2_signal, start_time, end_time, period):
...
@theme.command("list")
@click.option(
    "--names-only", is_flag=True, default=False, help="Print just the sorted theme name list"
)
@click.pass_context
...
@click.option("--start", required=True, help="ISO timestamp (T present) or YYYY-MM-DD (all-day)")
@click.option("--end", default=None, help="ISO timestamp or YYYY-MM-DD (matches start's format)")
@click.option("--description", default=None)
@click.option("--location", default=None)
@click.option("--rrule", default=None, help="Recurrence rule, e.g. FREQ=WEEKLY;BYDAY=MO,WE,FR")
@click.pass_context
def calendar_create(ctx, entity_id, summary, start, end, description, location, rrule):
    emit(
        ctx,
...
@click.option("--recurrence-id", default=None)
@click.option(
    "--recurrence-range", default=None, type=click.Choice(["THISANDFUTURE", "THISANDPRIOR"])
)
...
@click.option("--option", "options_kv", multiple=True, help="Engine option key=value (repeatable)")
@click.pass_context
def tts_speak(ctx, tts_entity, message, media_player_entity, language, no_cache, options_kv):
...
@click.argument("entry_id")
@click.argument("ident", metavar="SUBENTRY_ID_OR_TITLE")
@click.pass_context
...
@click.argument("ident", metavar="SUBENTRY_ID_OR_TITLE")
@click.option(
    "--set", "set_pairs", multiple=True, help="Field override key=value (repeatable, JSON-aware)"
)
@click.option(
    "--data-file",
    type=click.Path(exists=True, dir_okay=False),
...
@click.option(
    "--pattern", default=None, help="Substring filter on name OR full_name (case-insensitive)"
)
@click.pass_context
...
@click.argument("ident")
@click.option("--version", default=None, help="Specific version tag (default: latest)")
...
@click.option(
    "--domain", default=None, help="Restrict to parent entries in this integration domain"
)
@click.pass_context
...
@click.argument("url_path")
@click.option(
    "--pointer", required=True, help="Card pointer, e.g. 'views[3]/cards[11]/cards[0]/cards[0]'"
)
@click.option(
    "--set", "set_pairs", multiple=True, help="Field override key=value (repeatable, JSON-aware)"
)
...
def lovelace_card_patch(ctx, url_path, pointer, set_pairs, data_file, strict, dry_run):
    ...
    if dry_run:
        ...
        emit(ctx, {"pointer": pointer, "dry_run": True, "before": before, "would_set": fields})
        return
    ...
    emit(ctx, {"pointer": pointer, "patched_fields": list(fields), "result": res})
...
@click.option(
    "--index", type=int, default=None, help="Position in destination cards[] (default: append)"
)
@click.pass_context
def lovelace_card_move(ctx, url_path, src_pointer, dest_parent_pointer, index):
    ...
    cfg = lovelace_core.get_dashboard_config(client, url_path)
    try:
        lovelace_card_ops_core.move_card(cfg, src_pointer, dest_parent_pointer, index=index)
    except (KeyError, IndexError, ValueError) as e:
        _abort(str(e))
    lovelace_core.save_dashboard_config(client, url_path, cfg)
    emit(ctx, {"moved": src_pointer, "to": dest_parent_pointer, "at_index": index})
...
@click.option(
    "--columns", type=int, default=None, help="Grid column count (only with --stack-type grid)"
)
...
def lovelace_card_wrap(ctx, url_path, pointers, stack_type, columns, title):
    ...
    try:
        ...
        _abort(str(e))
    lovelace_core.save_dashboard_config(client, url_path, cfg)
    emit(ctx, {"wrapped": list(pointers), "stack_type": stack_type, "new_pointer": new_ptr})


@lovelace_card.command("wrap-conditional")
@click.argument("url_path")
@click.argument("pointer")
@click.option("--entity", required=True, help="Entity to gate on")
@click.option("--state", required=True, help="State value the entity must equal")
@click.pass_context
...
@click.argument("pointer")
@click.option(
    "--substitute", "-s", "subs", multiple=True, help="Regex substitution old=new (repeatable)"
)
@click.option(
    "--index-offset",
    default=1,
...
def lovelace_card_duplicate(ctx, url_path, pointer, subs, index_offset):
    ...
    try:
        new_ptr = lovelace_card_ops_core.duplicate_card(
            cfg,
            pointer,
            substitutions=sub_map or None,
            index_offset=index_offset,
        )
    except (KeyError, IndexError, ValueError) as e:
        _abort(str(e))
    lovelace_core.save_dashboard_config(client, url_path, cfg)
    emit(ctx, {"duplicated": pointer, "new_pointer": new_ptr, "substitutions": sub_map})
...
@click.option(
    "--css-file", type=click.Path(exists=True, dir_okay=False), help="Read CSS from a file instead"
)
@click.option(
    "--target",
    default="root",
...
@click.option("--clear", is_flag=True, help="Remove all card_mod from this card instead")
@click.pass_context
def lovelace_card_style(ctx, url_path, pointer, css, css_file, target, clear):
    ...
    try:
        ...
        _abort(str(e))
    lovelace_core.save_dashboard_config(client, url_path, cfg)
    emit(ctx, {"styled": pointer, "target": target, "css_lines": len(css_text.splitlines())})
...
@click.option(
    "--dashboard", "url_path", default=None, help="Limit to one dashboard (default: scan all)"
)
@click.option("--installed", is_flag=True, help="Cross-reference custom types against HACS plugins")
@click.pass_context
...
@click.option(
    "--set", "set_pairs", multiple=True, help="Builder kwarg as key=value (JSON-aware, repeatable)"
)
@click.option(
    "--data-file",
    type=click.Path(exists=True, dir_okay=False),
...
@click.option("--position", type=int, default=None, help="Insert at this index in parent's cards[]")
@click.option(
    "--dry-run", is_flag=True, default=False, help="Build and print the card without saving"
)
@click.pass_context
def lovelace_card_create(
    ctx, url_path, parent_pointer, card_type, set_pairs, data_file, position, dry_run
):
    ...
    try:
        ...
        _abort(str(e))
    lovelace_core.save_dashboard_config(client, url_path, cfg)
    emit(ctx, {"created": new_card.get("type"), "into": parent_pointer, "position": position})
...
def lovelace_section_list(ctx, url_path, view_path):
    ...
    emit(
        ctx,
        [
            {"index": i, "type": s.get("type"), "cards": len(s.get("cards", []))}
...
@click.option("--index", type=int, default=None, help="Insert at this position (default: append)")
@click.pass_context
...
@click.option("--exclude", "-x", multiple=True, help="Substring patterns to skip (repeatable)")
@click.option("--backup", is_flag=True, default=False, help="Backup before each install")
@click.option("--dry-run", is_flag=True, default=False)
@click.pass_context
...
def entity_enable(ctx, entity_id):
    ...
    emit(ctx, registry_core.update_entity(make_client(ctx), entity_id, disabled_by=None))
...
@click.option("--repack", is_flag=True, default=False, help="VACUUM the DB after purge (slow)")
@click.option(
    "--apply-filter",
    is_flag=True,
    default=False,
...
def recorder_purge(ctx, keep_days, repack, apply_filter):
    ...
    emit(
        ctx,
        recorder_core.purge(
            make_client(ctx), keep_days=keep_days, repack=repack, apply_filter=apply_filter
        ),
    )


@recorder.command("purge-entities")
@click.option("--entity", "entity_ids", multiple=True, help="Exact entity_id to wipe (repeatable)")
@click.option(
    "--domain", "domains", multiple=True, help="Wipe every entity in this domain (repeatable)"
)
@click.option(
    "--glob",
    "entity_globs",
...
@click.option("--days", type=int, default=None, help="Keep this many days; older rows go")
...
def recorder_purge_entities(ctx, entity_ids, domains, entity_globs, days):
    ...
    emit(
        ...
        recorder_core.purge_entities(
            ...
            entity_globs=list(entity_globs),
            days=days,
        ),
    )
...
@click.argument("entity_id")
@click.option("--transition", type=float, default=None, help="Fade duration in seconds")
@click.pass_context
def scene_activate(ctx, entity_id, transition):
    """Activate a stored scene by entity_id."""
    emit(ctx, scenes_core.activate(make_client(ctx), entity_id, transition=transition))
...
def scene_apply(ctx, entity_pairs, transition):
    ...
    emit(ctx, scenes_core.apply(make_client(ctx), entities=entities, transition=transition))


@scene.command("create")
@click.argument("scene_id")
@click.option(
    "--entity", "entity_pairs", multiple=True, help="Explicit entity_id=state pair (repeatable)"
)
@click.option(
    "--snapshot",
    "snapshot",
...
def scene_create(ctx, scene_id, entity_pairs, snapshot):
    ...
    emit(
        ctx,
        scenes_core.create(
            make_client(ctx), scene_id=scene_id, entities=entities, snapshot_entities=snap
        ),
    )
...
def weather_list(ctx):
    ...
    rows = [
        s
        for s in (states or [])
        if isinstance(s, dict) and (s.get("entity_id") or "").startswith("weather.")
    ]
    emit(ctx, rows)
...
def weather_forecast(ctx, entity_id, forecast_type):
    ...
    emit(
        ctx,
        weather_advanced_core.get_forecasts(
            make_client(ctx),
            entity_id=entity_id,
...
def weather_forecast_subscribe(ctx, entity_id, forecast_type):
    ...
    emit(
        ctx,
        weather_advanced_core.subscribe_forecast(
            make_client(ctx),
            entity_id=entity_id,
            forecast_type=forecast_type,
        ),
...
@click.option(
    "--complete/--incomplete", "complete", default=None, help="Mark complete / incomplete"
)
@click.pass_context
def shopping_list_update(ctx, item_id, name, complete):
    ...
    emit(
        ctx,
        shopping_list_core.update_item(
            make_client(ctx),
            item_id=item_id,
            name=name,
            complete=complete,
...
def shopping_list_remove(ctx, item_id):
    ...
    emit(
        ctx,
        shopping_list_core.remove_item(
            make_client(ctx),
            item_id=item_id,
        ),
    )
...
def shopping_list_reorder(ctx, item_ids):
    ...
    emit(
        ctx,
        shopping_list_core.reorder_items(
            make_client(ctx),
...
def todo_add(ctx, entity_id, summary, due, description):
    ...
    emit(
        ctx,
        todos_core.add_item(
            make_client(ctx), entity_id, summary=summary, due=due, description=description
        ),
    )
...
@click.option("--status", default=None, type=click.Choice(["needs_action", "completed"]))
...
def todo_move(ctx, entity_id, uid, previous_uid):
    ...
    emit(ctx, todos_core.move_item(make_client(ctx), entity_id, uid=uid, previous_uid=previous_uid))
...
def lock_lock(ctx, entity_id, code):
    emit(
        ctx,
        service_shortcuts_core.lock_lock(
            make_client(ctx),
            entity_id,
            code=code,
        ),
...
def lock_unlock(ctx, entity_id, code):
    emit(
        ctx,
        service_shortcuts_core.lock_unlock(
            make_client(ctx),
            entity_id,
            code=code,
        ),
...
def lock_open(ctx, entity_id, code):
    ...
    emit(
        ctx,
        service_shortcuts_core.lock_open(
            make_client(ctx),
            entity_id,
            code=code,
        ),
...
def alarm_arm_away(ctx, entity_id, code):
    emit(
        ctx,
        service_shortcuts_core.alarm_arm_away(
            make_client(ctx),
            entity_id,
            code=code,
        ),
...
def alarm_arm_home(ctx, entity_id, code):
    emit(
        ctx,
        service_shortcuts_core.alarm_arm_home(
            make_client(ctx),
            entity_id,
            code=code,
        ),
...
def alarm_arm_night(ctx, entity_id, code):
    emit(
        ctx,
        service_shortcuts_core.alarm_arm_night(
            make_client(ctx),
            entity_id,
            code=code,
        ),
...
def alarm_arm_vacation(ctx, entity_id, code):
    emit(
        ctx,
        service_shortcuts_core.alarm_arm_vacation(
            make_client(ctx),
            entity_id,
            code=code,
        ),
...
@click.option("--skip-delay", is_flag=True, default=False, help="Skip the exit delay countdown")
@click.option(
    "--force",
    is_flag=True,
    default=False,
...
@click.argument("entity_id")
@click.option("--code", default=None, help="Alarmo user code (if required)")
@click.option("--skip-delay", is_flag=True, default=False, help="Skip the entry delay countdown")
@click.pass_context
...
@click.option(
    "--data-json", default=None, help="JSON partial config to merge (only fields you pass change)"
)
@click.option(
    "--dry-run", is_flag=True, default=False, help="Print the REST path and JSON body; send nothing"
)
@click.option(
    "--yes",
    "-y",
...
@click.option("--area-id", default=None, help="Pass to rename an existing area; omit to create new")
@click.option(
    "--modes-json",
    default=None,
...
def alarmo_area_delete(ctx, area_id, dry_run, yes):
    ...
    body = {"path": "alarmo/area", "json": {"area_id": area_id, "remove": True}}
    if dry_run:
        emit(ctx, {"dry_run": True, **body})
        return
    if not yes and not click.confirm(
        f"Delete Alarmo area {area_id!r}?",
        default=False,
    ):
        raise click.ClickException("aborted")
...
def alarmo_sensor_remove(ctx, entity_id, dry_run, yes):
    ...
    body = {"path": "alarmo/sensors", "json": {"entity_id": entity_id, "remove": True}}
    if dry_run:
        emit(ctx, {"dry_run": True, **body})
        return
    ...
    if not yes:
        ...
        if not click.confirm(
            f"Remove sensor {entity_id!r} from Alarmo — it will no longer trigger or block arming?",
            default=False,
        ):
            raise click.ClickException("aborted")
...
def alarmo_sensor_update(
        ...
        for m in parsed_modes:
            if m not in alarmo_core.SENSOR_ARM_MODES:
                raise click.UsageError(
                    f"invalid mode {m!r}; must be one of {', '.join(alarmo_core.SENSOR_ARM_MODES)}"
                )
    ...
    body = {"path": "alarmo/sensors", "json": {"entity_id": entity_id, **fields}}
    if dry_run:
        emit(ctx, {"dry_run": True, **body})
        return
    ...
    if not yes:
        ...
        if current is not None:
            ...
            click.echo(f"(sensor {entity_id!r} not currently registered)")
        if not click.confirm(
            f"Update sensor {entity_id!r} in Alarmo — fields: {', '.join(sorted(fields))}?",
            default=False,
        ):
            raise click.ClickException("aborted")

    emit(ctx, alarmo_core.sensor_update(make_client(ctx), entity_id, **fields))
...
def search(ctx, item_type, item_id):
    ...
    emit(
        ctx,
        singletons_core.search_related(
            make_client(ctx),
            item_type=item_type,
            item_id=item_id,
        ),
...
def entity_expose_list(ctx, assistant):
    ...
    emit(
        ctx,
        expose_entity_core.list_exposed(
            make_client(ctx),
            assistant=assistant,
        ),
    )
...
@click.option("--entity", "entity_ids", multiple=True, required=True, help="Entity id (repeatable)")
@click.option(
    "--expose/--hide", "should_expose", default=True, help="Expose (default) or hide the entities"
)
@click.pass_context
def entity_expose_set(ctx, assistants, entity_ids, should_expose):
    ...
    emit(
        ctx,
        expose_entity_core.expose_entity(
            make_client(ctx),
...
def entity_expose_new_set(ctx, assistant, expose_new):
    emit(
        ctx,
        expose_entity_core.set_expose_new_entities(
            make_client(ctx),
            assistant=assistant,
...
def camera_stream(ctx, entity_id, stream_format):
    ...
    emit(ctx, camera_ws_core.stream(make_client(ctx), entity_id=entity_id, format=stream_format))
...
@click.argument("entity_id")
@click.option(
    "--preload-stream/--no-preload-stream",
    "preload_stream",
    default=None,
    help="Preload the stream on frontend load",
)
@click.option("--orientation", type=int, default=None, help="EXIF orientation code (1-8)")
@click.pass_context
...
def device_automation_triggers(ctx, device_id):
    ...
    emit(
        ctx,
        device_automation_core.list_triggers(
            make_client(ctx),
            device_id=device_id,
        ),
    )
...
def device_automation_conditions(ctx, device_id):
    ...
    emit(
        ctx,
        device_automation_core.list_conditions(
            make_client(ctx),
            device_id=device_id,
        ),
    )
...
def device_automation_actions(ctx, device_id):
    ...
    emit(
        ctx,
        device_automation_core.list_actions(
            make_client(ctx),
            device_id=device_id,
        ),
    )
...
def device_automation_summary(ctx, device_id):
    ...
    emit(
        ctx,
        device_automation_core.summarise_device(
            make_client(ctx),
            device_id=device_id,
        ),
    )
...
def assist_agents(ctx, country, language):
    ...
    emit(
        ctx,
        conversation_advanced_core.list_agents(
            make_client(ctx),
            country=country,
            language=language,
        ),
...
def assist_sentences(ctx, language):
    ...
    emit(
        ctx,
        conversation_advanced_core.list_sentences(
            make_client(ctx),
            language=language,
        ),
    )
...
def assist_debug(ctx, sentence, language):
    ...
    emit(
        ctx,
        conversation_advanced_core.debug_agent(
            make_client(ctx),
            sentence=sentence,
            language=language,
        ),
...
def assist_satellite_config(ctx, entity_id):
    ...
    emit(
        ctx,
        assist_satellite_core.get_configuration(
            make_client(ctx),
            entity_id=entity_id,
        ),
    )
...
def assist_satellite_wake_words(ctx, entity_id, wake_word_ids):
    ...
    emit(
        ctx,
        assist_satellite_core.set_wake_words(
            make_client(ctx),
            entity_id=entity_id,
...
def assist_satellite_test(ctx, entity_id):
    ...
    emit(
        ctx,
        assist_satellite_core.test_connection(
            make_client(ctx),
            entity_id=entity_id,
        ),
    )
...
def mobile_app_confirm_push(ctx, webhook_id, confirm_id):
    ...
    emit(
        ctx,
        mobile_app_core.confirm_push_notification(
            make_client(ctx),
...
def media_browse(ctx, media_content_id):
    ...
    emit(
        ctx,
        media_source_core.browse_media(
            make_client(ctx),
...
def media_resolve(ctx, media_content_id):
    ...
    emit(
        ctx,
        media_source_core.resolve_media(
            make_client(ctx),
            media_content_id=media_content_id,
        ),
    )
...
def media_remove(ctx, media_content_id):
    ...
    emit(
        ctx,
        media_source_core.local_source_remove(
            make_client(ctx),
            media_content_id=media_content_id,
        ),
    )
...
@click.argument("path")
@click.option(
    "--expires", type=int, default=30, help="Seconds until the signed URL expires (default 30)"
)
@click.pass_context
...
def auth_tokens_delete(ctx, refresh_token_id):
    ...
    emit(
        ctx,
        auth_tokens_core.delete_refresh_token(
            make_client(ctx),
...
def auth_tokens_delete_all(ctx, delete_current, token_type):
    ...
    emit(
        ctx,
        auth_tokens_core.delete_all_refresh_tokens(
            make_client(ctx),
...
def auth_tokens_set_expiry(ctx, refresh_token_id, enable_expiry):
    ...
    emit(
        ctx,
        auth_tokens_core.set_refresh_token_expiry(
            make_client(ctx),
            refresh_token_id=refresh_token_id,
...
@click.option("--name", default=None)
@click.option("--group", "group_ids", multiple=True, help="Replace group membership (repeatable)")
...
def auth_user_update(ctx, user_id, name, group_ids, local_only, is_active):
    ...
    if name is None and not group_ids and local_only is None and is_active is None:
        _abort("provide at least one of --name/--group/--local-only/--active")
    emit(
        ctx,
        user_admin_core.update_user(
            make_client(ctx),
            user_id=user_id,
            name=name,
            group_ids=list(group_ids) or None,
...
def auth_user_credential_create(ctx, user_id, username, password):
    ...
    emit(
        ctx,
        user_admin_core.create_credential(
            make_client(ctx),
            user_id=user_id,
            username=username,
            password=password,
...
def auth_user_credential_delete(ctx, username):
    ...
    emit(
        ctx,
        user_admin_core.delete_credential(
            make_client(ctx),
            username=username,
        ),
    )
...
@click.option(
    "--new-password", required=True, prompt=True, hide_input=True, confirmation_prompt=True
)
@click.pass_context
def auth_user_change_password(ctx, current_password, new_password):
    ...
    emit(
        ctx,
        user_admin_core.change_password(
            make_client(ctx),
...
def category_delete(ctx, scope, category_id):
    emit(
        ctx,
        categories_core.delete_category(
            make_client(ctx),
            scope=scope,
            category_id=category_id,
        ),
...
@click.option(
    "--integration", default=None, help="Integration domain (mutually exclusive with --namespace)"
)
@click.option(
    "--namespace",
    default=None,
...
@click.argument(
    "level",
    type=click.Choice(
        ["debug", "info", "warning", "error", "critical", "fatal", "notset"], case_sensitive=False
    ),
)
@click.option(
    "--persistence",
...
def logger_level_set(ctx, integration, level, persistence):
    ...
    emit(
        ctx,
        logger_ws_core.integration_log_level(
            make_client(ctx),
            integration=integration,
            level=level.lower(),
...
def system_analytics_set(ctx, preferences_json):
    ...
    emit(
        ctx,
        system_ops_core.set_analytics_preferences(
            make_client(ctx),
            preferences=prefs,
        ),
    )
...
def system_app_credentials_entry(ctx, config_entry_id):
    ...
    emit(
        ctx,
        system_ops_core.application_credentials_config_entry(
            make_client(ctx),
...
def system_issue_get_data(ctx, domain, issue_id):
    ...
    emit(
        ctx,
        system_ops_core.get_issue_data(
            make_client(ctx),
            domain=domain,
            issue_id=issue_id,
        ),
...
@click.option(
    "--duration", type=int, default=60, help="Permit window in seconds (1-254, default 60)"
)
@click.option("--ieee", default=None, help="Restrict permit to a specific device IEEE address")
@click.pass_context
def system_zha_permit_join(ctx, duration, ieee):
    """Open the ZHA Zigbee network for new device joins."""
    emit(
        ctx,
        singletons_core.zha_devices_permit(
            make_client(ctx),
            duration=duration,
            ieee=ieee,
        ),
...
@click.option("--brightness-pct", "brightness_pct", type=float, default=None, help="0..100")
@click.option("--kelvin", type=int, default=None)
@click.option("--color-temp-kelvin", "color_temp_kelvin", type=int, default=None)
@click.option("--rgb", "rgb_color", default=None, help="Comma-separated r,g,b (e.g. 255,128,0)")
@click.option("--rgbw", "rgbw_color", default=None, help="Comma-separated r,g,b,w")
@click.option("--rgbww", "rgbww_color", default=None, help="Comma-separated r,g,b,cw,ww")
@click.option("--xy", "xy_color", default=None, help="Comma-separated x,y")
@click.option("--hs", "hs_color", default=None, help="Comma-separated hue,saturation")
@click.option("--color-name", "color_name", default=None)
@click.option("--effect", default=None)
@click.option("--flash", default=None, type=click.Choice(["short", "long"], case_sensitive=False))
@click.option("--transition", type=float, default=None)
@click.option("--profile", default=None)
@click.option("--white", type=int, default=None, help="White-channel value 0..255")
@click.pass_context
def light_on(
    ctx,
    entity_id,
    ...
    emit(
        ctx,
        entity_control_core.light_turn_on(
            make_client(ctx),
            entity_id,
...
def light_off(ctx, entity_id, transition, flash):
    ...
    emit(
        ctx,
        entity_control_core.light_turn_off(
            make_client(ctx),
            entity_id,
            transition=transition,
            flash=flash.lower() if flash else None,
...
@click.option("--transition", type=float, default=None)
@click.pass_context
def light_toggle_cmd(ctx, entity_id, brightness, brightness_pct, kelvin, rgb_color, transition):
    """Toggle a light."""
    rgb_parsed = [int(p.strip()) for p in rgb_color.split(",")] if rgb_color else None
    emit(
        ctx,
        entity_control_core.light_toggle(
            make_client(ctx),
            entity_id,
            brightness=brightness,
            brightness_pct=brightness_pct,
...
def mp_play_pause(ctx, entity_id):
    emit(ctx, entity_control_core.media_player_play_pause(make_client(ctx), entity_id))
...
def mp_previous(ctx, entity_id):
    emit(ctx, entity_control_core.media_player_previous(make_client(ctx), entity_id))
...
def mp_volume_set(ctx, entity_id, volume):
    ...
    emit(
        ctx,
        entity_control_core.media_player_volume_set(
            make_client(ctx),
            entity_id,
            volume=volume,
        ),
...
def mp_volume_up(ctx, entity_id):
    emit(ctx, entity_control_core.media_player_volume_up(make_client(ctx), entity_id))
...
def mp_volume_down(ctx, entity_id):
    emit(ctx, entity_control_core.media_player_volume_down(make_client(ctx), entity_id))


@media_player_grp.command("mute")
@click.argument("entity_id")
@click.option("--off", "off", is_flag=True, default=False, help="Unmute (default action: mute)")
@click.pass_context
def mp_mute(ctx, entity_id, off):
    emit(
        ctx,
        entity_control_core.media_player_mute(
            make_client(ctx),
            entity_id,
            mute=not off,
        ),
...
def mp_select_source(ctx, entity_id, source):
    emit(
        ctx,
        entity_control_core.media_player_select_source(
            make_client(ctx),
            entity_id,
            source=source,
        ),
...
def mp_select_sound_mode(ctx, entity_id, sound_mode):
    emit(
        ctx,
        entity_control_core.media_player_select_sound_mode(
            make_client(ctx),
            entity_id,
...
@click.option("--enqueue", default=None, help="play|add|next|replace, or true/false")
@click.option(
    "--announce/--no-announce", default=None, help="Use the announce surface where supported"
)
@click.option("--extra", default=None, help="JSON-encoded extra payload")
@click.pass_context
def mp_play_media(ctx, entity_id, media_content_id, media_content_type, enqueue, announce, extra):
    parsed_extra = json.loads(extra) if extra else None
    emit(
        ctx,
        entity_control_core.media_player_play_media(
            make_client(ctx),
            entity_id,
            media_content_id=media_content_id,
...
@click.argument("entity_id")
@click.option("--off", "off", is_flag=True, default=False, help="Turn shuffle off (default: on)")
@click.pass_context
def mp_shuffle(ctx, entity_id, off):
    emit(
        ctx,
        entity_control_core.media_player_shuffle(
            make_client(ctx),
            entity_id,
            shuffle=not off,
        ),
...
def mp_repeat(ctx, entity_id, mode):
    emit(
        ctx,
        entity_control_core.media_player_repeat(
            make_client(ctx),
            entity_id,
            repeat=mode.lower(),
        ),
...
def mp_clear_playlist(ctx, entity_id):
    emit(
        ctx,
        entity_control_core.media_player_clear_playlist(
            make_client(ctx),
            entity_id,
        ),
    )
...
def mp_turn_on(ctx, entity_id):
    emit(ctx, entity_control_core.media_player_turn_on(make_client(ctx), entity_id))
...
def mp_turn_off(ctx, entity_id):
    emit(ctx, entity_control_core.media_player_turn_off(make_client(ctx), entity_id))


@media_player_grp.command("join")
@click.argument("entity_id")
@click.option(
    "--member", "members", multiple=True, required=True, help="Group member entity_id (repeatable)"
)
@click.pass_context
def mp_join(ctx, entity_id, members):
    emit(
        ctx,
        entity_control_core.media_player_join(
            make_client(ctx),
            entity_id,
...
@click.option("--hvac-mode", "hvac_mode", default=None, help="Optional hvac_mode to set atomically")
@click.pass_context
def climate_set_temp(ctx, entity_id, temperature, target_temp_high, target_temp_low, hvac_mode):
    emit(
        ctx,
        entity_control_core.climate_set_temperature(
            make_client(ctx),
            entity_id,
...
def climate_set_hvac(ctx, entity_id, hvac_mode):
    ...
    emit(
        ctx,
        entity_control_core.climate_set_hvac_mode(
            make_client(ctx),
            entity_id,
            hvac_mode=hvac_mode,
        ),
...
def climate_set_fan(ctx, entity_id, fan_mode):
    emit(
        ctx,
        entity_control_core.climate_set_fan_mode(
            make_client(ctx),
            entity_id,
            fan_mode=fan_mode,
        ),
...
def climate_set_preset(ctx, entity_id, preset_mode):
    emit(
        ctx,
        entity_control_core.climate_set_preset_mode(
            make_client(ctx),
            entity_id,
...
def climate_set_hum(ctx, entity_id, humidity):
    emit(
        ctx,
        entity_control_core.climate_set_humidity(
            make_client(ctx),
            entity_id,
            humidity=humidity,
        ),
...
def climate_set_swing(ctx, entity_id, swing_mode):
    emit(
        ctx,
        entity_control_core.climate_set_swing_mode(
            make_client(ctx),
            entity_id,
...
def cover_set_pos(ctx, entity_id, position):
    ...
    emit(
        ctx,
        entity_control_core.cover_set_position(
            make_client(ctx),
            entity_id,
            position=position,
        ),
...
def cover_set_tilt_cmd(ctx, entity_id, tilt_position):
    emit(
        ctx,
        entity_control_core.cover_set_tilt(
            make_client(ctx),
            entity_id,
...
def fan_on_cmd(ctx, entity_id, percentage, preset_mode):
    emit(
        ctx,
        entity_control_core.fan_turn_on(
            make_client(ctx),
            entity_id,
...
def fan_set_pct(ctx, entity_id, percentage):
    emit(
        ctx,
        entity_control_core.fan_set_percentage(
            make_client(ctx),
            entity_id,
            percentage=percentage,
        ),
...
def fan_set_preset_cmd(ctx, entity_id, preset_mode):
    emit(
        ctx,
        entity_control_core.fan_set_preset(
            make_client(ctx),
            entity_id,
            preset_mode=preset_mode,
        ),
...
@click.argument("entity_id")
@click.argument("direction", type=click.Choice(["forward", "reverse"], case_sensitive=False))
@click.pass_context
def fan_set_dir(ctx, entity_id, direction):
    emit(
        ctx,
        entity_control_core.fan_set_direction(
            make_client(ctx),
            entity_id,
...
@click.argument("entity_id")
@click.option("--off", "off", is_flag=True, default=False, help="Stop oscillation (default: start)")
@click.pass_context
def fan_osc(ctx, entity_id, off):
    emit(
        ctx,
        entity_control_core.fan_oscillate(
            make_client(ctx),
            entity_id,
            oscillating=not off,
        ),
...
def fan_inc(ctx, entity_id, percentage_step):
    emit(
        ctx,
        entity_control_core.fan_increase(
            make_client(ctx),
            entity_id,
...
def fan_dec(ctx, entity_id, percentage_step):
    emit(
        ctx,
        entity_control_core.fan_decrease(
            make_client(ctx),
            entity_id,
            percentage_step=percentage_step,
        ),
...
def vacuum_return_cmd(ctx, entity_id):
    emit(ctx, entity_control_core.vacuum_return_to_base(make_client(ctx), entity_id))
...
def vacuum_clean_spot_cmd(ctx, entity_id):
    emit(ctx, entity_control_core.vacuum_clean_spot(make_client(ctx), entity_id))
...
def vacuum_fan_speed_cmd(ctx, entity_id, fan_speed):
    emit(
        ctx,
        entity_control_core.vacuum_set_fan_speed(
            make_client(ctx),
            entity_id,
            fan_speed=fan_speed,
        ),
...
@click.option("--params", default=None, help="JSON-encoded params payload")
@click.pass_context
def vacuum_send_cmd(ctx, entity_id, command, params):
    parsed = json.loads(params) if params else None
    emit(
        ctx,
        entity_control_core.vacuum_send_command(
            make_client(ctx),
            entity_id,
            command=command,
            params=parsed,
...
def humidifier_on(ctx, entity_id):
    emit(ctx, entity_control_core.humidifier_turn_on(make_client(ctx), entity_id))
...
def humidifier_off(ctx, entity_id):
    emit(ctx, entity_control_core.humidifier_turn_off(make_client(ctx), entity_id))
...
def humidifier_toggle_cmd(ctx, entity_id):
    emit(ctx, entity_control_core.humidifier_toggle(make_client(ctx), entity_id))
...
def humidifier_set_hum(ctx, entity_id, humidity):
    emit(
        ctx,
        entity_control_core.humidifier_set_humidity(
            make_client(ctx),
            entity_id,
            humidity=humidity,
        ),
...
def humidifier_set_mode_cmd(ctx, entity_id, mode):
    emit(
        ctx,
        entity_control_core.humidifier_set_mode(
            make_client(ctx),
            entity_id,
            mode=mode,
        ),
...
def wh_on(ctx, entity_id):
    emit(ctx, entity_control_core.water_heater_turn_on(make_client(ctx), entity_id))
...
def wh_off(ctx, entity_id):
    emit(ctx, entity_control_core.water_heater_turn_off(make_client(ctx), entity_id))
...
def wh_set_temp(ctx, entity_id, temperature):
    emit(
        ctx,
        entity_control_core.water_heater_set_temperature(
            make_client(ctx),
            entity_id,
            temperature=temperature,
        ),
...
def wh_set_op_mode(ctx, entity_id, operation_mode):
    emit(
        ctx,
        entity_control_core.water_heater_set_operation_mode(
            make_client(ctx),
            entity_id,
...
@click.argument("entity_id")
@click.option("--off", "off", is_flag=True, default=False, help="Turn away mode off (default: on)")
@click.pass_context
def wh_set_away(ctx, entity_id, off):
    emit(
        ctx,
        entity_control_core.water_heater_set_away_mode(
            make_client(ctx),
            entity_id,
            away_mode=not off,
        ),
...
def valve_set_pos(ctx, entity_id, position):
    emit(
        ctx,
        entity_control_core.valve_set_position(
            make_client(ctx),
            entity_id,
            position=position,
        ),
...
@click.option("--volume", "volume_level", type=float, default=None, help="0.0..1.0")
@click.pass_context
def siren_on_cmd(ctx, entity_id, duration, tone, volume_level):
    emit(
        ctx,
        entity_control_core.siren_turn_on(
            make_client(ctx),
            entity_id,
            duration=duration,
            tone=tone,
...
@click.argument("entity_id")
@click.option("--activity", default=None, help="Harmony-style activity to switch to")
@click.pass_context
def remote_on(ctx, entity_id, activity):
    emit(
        ctx,
        entity_control_core.remote_turn_on(
            make_client(ctx),
            entity_id,
            activity=activity,
        ),
...
@click.pass_context
def remote_send_cmd(ctx, entity_id, commands, device, num_repeats, delay_secs, hold_secs):
    cmd: str | list[str] = list(commands) if len(commands) > 1 else commands[0]
    emit(
        ctx,
        entity_control_core.remote_send_command(
            make_client(ctx),
            entity_id,
            command=cmd,
            device=device,
...
@click.argument("entity_id")
@click.option(
    "--command", "-c", "commands", multiple=True, help="Command name (repeatable; learns as a list)"
)
@click.option("--device", default=None)
@click.option("--command-type", "command_type", default=None, help="ir|rf|... (backend-specific)")
@click.option("--alternative/--no-alternative", default=None)
@click.option("--timeout", type=float, default=None)
@click.pass_context
def remote_learn_cmd(ctx, entity_id, commands, device, command_type, alternative, timeout):
    cmd: str | list[str] | None = None
    if commands:
        cmd = list(commands) if len(commands) > 1 else commands[0]
    emit(
        ctx,
        entity_control_core.remote_learn_command(
            make_client(ctx),
            entity_id,
            command=cmd,
            device=device,
...
def remote_delete_cmd(ctx, entity_id, commands, device):
    ...
    emit(
        ctx,
        entity_control_core.remote_delete_command(
            make_client(ctx),
            entity_id,
            command=cmd,
            device=device,
...
def number_set(ctx, entity_id, value):
    emit(
        ctx,
        entity_control_core.number_set_value(
            make_client(ctx),
            entity_id,
            value=value,
        ),
...
def select_set(ctx, entity_id, option):
    ...
    emit(
        ctx,
        entity_control_core.select_select_option(
            make_client(ctx),
            entity_id,
            option=option,
        ),
...
def select_next_cmd(ctx, entity_id, cycle):
    emit(
        ctx,
        entity_control_core.select_next(
            make_client(ctx),
            entity_id,
            cycle=cycle,
        ),
...
def select_prev_cmd(ctx, entity_id, cycle):
    emit(
        ctx,
        entity_control_core.select_previous(
            make_client(ctx),
            entity_id,
            cycle=cycle,
        ),
...
def text_set(ctx, entity_id, value):
    emit(
        ctx,
        entity_control_core.text_set_value(
            make_client(ctx),
            entity_id,
            value=value,
        ),
...
@click.option("--title", default=None)
@click.option(
    "--service", default="notify", help="Which notify.<service> to call (default: notify)"
)
@click.option("--target", "targets", multiple=True, help="Target id (repeatable)")
...
def notify_send_cmd(ctx, message, title, service, targets, data):
    ...
    emit(
        ctx,
        entity_control_core.notify_send(
            make_client(ctx),
            message=message,
            title=title,
            target=target,
...
@click.option(
    "--title-contains", default=None, help="Case-insensitive substring filter on entry title"
)
@click.option("--state", default=None, help="Filter by config-entry state (loaded/not_loaded/...)")
@click.pass_context
def powercalc_list(ctx, title_contains, state):
    """List powercalc config entries."""
    emit(
        ctx,
        powercalc_core.list_entries(
            make_client(ctx),
...
@click.option(
    "--source", "source_entity", required=True, help="Source entity_id powering the calculation"
)
...
@click.option(
    "--standby", "standby_power", type=float, default=0, help="Standby power in W (default 0)"
)
@click.option(
    "--no-energy-sensor",
...
@click.option(
    "--group", "groups", multiple=True, help="Powercalc group entry_id to auto-join (repeatable)"
)
@click.pass_context
def powercalc_create(
    ctx,
    source_entity,
    name,
    power,
    ...
    emit(
        ctx,
        powercalc_core.create_virtual_power(
            make_client(ctx),
...
def powercalc_set_power(ctx, entry_id, power):
    ...
    emit(
        ctx,
        powercalc_core.set_fixed_power(
            make_client(ctx),
            entry_id,
            power=power,
        ),
...
def powercalc_reload(ctx, entry_ids):
    ...
    emit(
        ctx,
        powercalc_core.reload_groups_for_member(
            make_client(ctx),
            parent_entry_ids=list(entry_ids),
        ),
    )
...
def powercalc_group_members(ctx, sensor_entity_id):
    ...
    emit(
        ctx,
        powercalc_core.get_group_members(
            make_client(ctx),
            sensor_entity_id,
        ),
    )
...
@click.option(
    ...
    default=True,
    flag_value=False,
    help="Skip the read-back persistence check",
)
@click.pass_context
def powercalc_group_add_members(
    ctx, entry_id, member_sensors, power_entities, energy_entities, verify
):
    ...
    emit(
        ctx,
        powercalc_core.add_group_members(
            make_client(ctx),
            entry_id,
...
@click.option("--entry-id", required=True, help="Group's config-entry id")
@click.option(
    "--member", "member_sensors", multiple=True, help="Member config-entry id to drop (repeatable)"
)
@click.option(
    "--power-entity",
    "power_entities",
...
@click.pass_context
def powercalc_group_remove_members(
    ctx, entry_id, member_sensors, power_entities, energy_entities, verify
):
    ...
    emit(
        ctx,
        powercalc_core.remove_group_members(
            make_client(ctx),
            entry_id,
            member_sensors=list(member_sensors) or None,
            power_entities=list(power_entities) or None,
...
@click.option(
    "--sub-group", "sub_groups", multiple=True, help="Replacement sub_groups list (repeatable)"
)
@click.option(
    "--no-verify",
    "verify",
...
@click.pass_context
def powercalc_group_set_members(
    ctx, entry_id, member_sensors, power_entities, energy_entities, sub_groups, verify
):
    ...
    emit(
        ctx,
        powercalc_core.set_group_members(
            make_client(ctx),
            entry_id,
...
def powercalc_group_groups_of(ctx, entry_ids, power_entities, energy_entities):
    ...
    emit(
        ctx,
        powercalc_core.find_groups_containing(
            make_client(ctx),
...
@click.option("--platform", default=None, help="Filter to entities whose registry platform matches")
@click.option(
    "--summary/--no-summary",
    default=True,
...
def entity_restored(ctx, platform, summary):
    ...
    if summary and not ctx.obj.get("as_json"):
        ...
        emit(ctx, {"total": len(rows), "by_platform": dict(by_plat.most_common())})
        return
    emit(ctx, rows)
...
@click.option("--platform", default=None, help="Only delete entries with this registry platform")
@click.option(
    "--restored",
    is_flag=True,
    default=False,
...
@click.option(
    ...
    default=None,
    type=click.Choice(
        ["integration", "user", "device", "config_entry", "hass", "any"], case_sensitive=False
    ),
...
@click.option(
    "--dry-run/--apply", default=True, help="Default: dry run. Pass --apply to actually delete."
)
@click.option(
    "--protect-user-disabled/--no-protect-user-disabled",
    default=True,
    help="Never delete user-disabled entries (default: on)",
)
@click.pass_context
def entity_prune(
    ctx, platform, restored, orphan, disabled_by, entity_ids, dry_run, protect_user_disabled
):
    ...
    if entity_ids:
        ...
        if protect_user_disabled:
            ...
            user_disabled = {e["entity_id"] for e in reg if e.get("disabled_by") == "user"}
        ...
        if restored:
            ...
            live_ids = {
                s["entity_id"] for s in states if s.get("attributes", {}).get("restored") is True
            }
...
@click.option("--hours", type=float, default=24, help="Window (default 24h)")
@click.option(
    "--domain",
    "domains",
    multiple=True,
...
@click.option("--hours", type=float, default=24, help="Window in hours (default 24)")
@click.option("--top", "top_n", type=int, default=8, help="How many groups to rank (default 8)")
@click.pass_context
...
@click.option(
    ...
    "service_on",
    required=True,
    help="Service to invoke to put the device under load (e.g. light.turn_on, switch.turn_on)",
)
@click.option(
    "--target", required=True, help="entity_id to target (or a JSON dict for richer targets)"
)
@click.option(
    "--service-off",
    "service_off",
...
@click.option("--samples", type=int, default=6, help="Samples per measurement window (default 6)")
@click.option(
    "--max-variance-w",
    "max_variance_w",
    type=float,
...
@click.option(
    ...
    type=int,
    default=2,
    help="Extra attempts for a noisy window before giving up (default 2)",
)
@click.option(
    "--apply",
    "apply_",
...
def powercalc_calibrate(
    ...
    emit(
        ctx,
        powercalc_calibration_core.calibrate(
            make_client(ctx),
            entry_id,
            **kw,
        ),
...
@click.option(
    "--attribute", required=True, help="State attribute keying the template (e.g. percentage)"
)
@click.option(
    "--service-set",
    "service_set",
...
@click.option(
    "--states",
    required=True,
    help="Comma-separated list of values to walk through (e.g. 0,25,50,75,100)",
)
@click.option("--smart-meter", "smart_meter", default=None)
@click.option("--baseline-seconds", "baseline_seconds", type=float, default=20)
@click.option("--stabilisation-seconds", "stabilisation_seconds", type=float, default=15)
...
@click.option(
    ...
    type=int,
    default=2,
    help="Extra attempts for a noisy step before excluding it (default 2)",
)
@click.option(
    "--apply",
    "apply_",
...
@click.pass_context
def powercalc_calibrate_template(
    ctx,
    entry_id,
    source_entity,
    attribute,
    ...
    emit(
        ctx,
        powercalc_calibration_core.calibrate_template(
            make_client(ctx),
            entry_id,
            **kw,
        ),
...
@click.option("--hours", type=float, default=24 * 7, help="History window (default 168h = 7 days)")
@click.option(
    "--pre-seconds",
    "pre_window_seconds",
    type=float,
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Apply median deltas via powercalc set-power. Dry-run by default.",
)
@click.pass_context
def powercalc_auto_calibrate(
    ctx,
    smart_meter,
    hours,
    pre_window_seconds,
    ...
    emit(
        ctx,
        powercalc_calibration_core.auto_calibrate(
            make_client(ctx),
            **kw,
        ),
    )
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Write each fitted coefficient into the matching entry's fixed power. Dry-run by default.",
)
@click.pass_context
def powercalc_regress(
    ctx,
    smart_meter,
    hours,
    interval_seconds,
...
@click.argument("name")
@click.option(
    "--lat", "latitude", type=float, required=True, help="Centre latitude (decimal degrees)"
)
@click.option(
    "--lon", "longitude", type=float, required=True, help="Centre longitude (decimal degrees)"
)
@click.option(
    "--radius",
    type=float,
...
@click.argument("webhook_id")
@click.option(
    "--method", type=click.Choice(["POST", "PUT", "GET", "HEAD"]), default="POST", show_default=True
)
@click.option(
    "--data", default=None, help="JSON body to send (object). Mutually exclusive with --form-data"
)
@click.option(
    "--form-data",
    "form_pairs",
...
def image_list(ctx, include_attributes):
    ...
    emit(
        ctx,
        image_core.list_image_entities(
            make_client(ctx),
...
@click.option(
    "--body-json", default=None, help="Raw JSON for the full payload (overrides per-field flags)"
)
@click.option(
    "--from-file",
    "from_file",
...
@click.option("--last-attempted", "last_attempted", default=None, help="ISO-8601 timestamp")
@click.option("--last-completed", "last_completed", default=None, help="ISO-8601 timestamp")
@click.option("--automatic/--no-automatic", "automatic", default=None)
@click.confirmation_option(prompt="Update backup configuration?")
@click.pass_context
def backup_advanced_update_config(
    ctx,
    body_json,
    from_file,
    create_backup_json,
...
def backup_advanced_can_decrypt(ctx, backup_id, agent_id, password):
    ...
    emit(
        ctx,
        backup_advanced_core.can_decrypt_on_download(
            make_client(ctx),
            backup_id=backup_id,
            agent_id=agent_id,
            password=password,
...
@click.argument("entity_id")
@click.option("--body-json", default=None, help="Inline JSON for the event dict")
@click.option(
    "--from-file",
    "from_file",
    type=click.Path(exists=True, dir_okay=False),
    default=None,
    help="Read event JSON from a file",
)
...
@click.argument("entity_id")
@click.option("--body-json", default=None, help="Inline JSON for the event dict (must include uid)")
@click.option(
    "--from-file", "from_file", type=click.Path(exists=True, dir_okay=False), default=None
)
@click.option("--recurrence-id", default=None, help="ISO-8601 datetime for a recurring instance")
@click.option("--recurrence-range", default=None, type=click.Choice(["", "THISANDFUTURE"]))
@click.confirmation_option(prompt="Update this calendar event?")
@click.pass_context
def calendar_ws_update(ctx, entity_id, body_json, from_file, recurrence_id, recurrence_range):
...
def entity_get_registry(ctx, entity_id):
    ...
    emit(
        ctx,
        entity_registry_extras_core.get_entity_registry_entry(
            make_client(ctx),
            entity_id=entity_id,
        ),
    )
...
def entity_get_many(ctx, entity_ids):
    ...
    emit(
        ctx,
        entity_registry_extras_core.get_entity_registry_entries(
            make_client(ctx),
            entity_ids=list(entity_ids),
        ),
    )
...
def entity_list_for_display(ctx):
    ...
    emit(
        ctx,
        entity_registry_extras_core.list_entity_registry_for_display(
            make_client(ctx),
        ),
    )
...
def entity_remove_registry(ctx, entity_id):
    ...
    emit(
        ctx,
        entity_registry_extras_core.remove_entity_registry_entry(
            make_client(ctx),
            entity_id=entity_id,
        ),
    )
...
@click.option("--calendar", "calendar_json", default=None, help="JSON for calendar period dict")
@click.option("--rolling-window", default=None, help="JSON for rolling_window dict")
@click.option("--type", "types", multiple=True, help="Statistic types to include (repeatable)")
@click.option("--units", default=None, help="JSON dict of unit overrides per statistic_id")
@click.pass_context
def entity_statistic_during_period(
    ctx, statistic_id, fixed_period, calendar_json, rolling_window, types, units
):
    """Query statistics for one statistic_id over a time period (singular)."""
    emit(
        ctx,
        entity_registry_extras_core.statistic_during_period(
            make_client(ctx),
            statistic_id=statistic_id,
...
@click.option("--key", default=None, help="Single key to read (omit for the whole store)")
@click.pass_context
...
def frontend_set_user_data(ctx, key, value_str, from_file):
    ...
    emit(
        ctx,
        frontend_prefs_core.set_user_data(
            make_client(ctx),
            key=key,
            value=value,
        ),
...
@click.option("--template", "template_str", default=None, help="Inline Jinja2 template string")
@click.option(
    "--from-file",
    "from_file",
    type=click.Path(exists=True, dir_okay=False),
...
@click.option("--variables", default=None, help="JSON dict of extra variables")
@click.option("--timeout", type=float, default=None, help="Render timeout in seconds")
@click.pass_context
def frontend_render_template(ctx, template_str, from_file, variables, timeout):
    ...
    emit(
        ctx,
        frontend_prefs_core.render_template(
            make_client(ctx),
            template=tpl,
...
@click.option(
    "--co2-statistic-id", required=True, help="Recorder statistic id for the CO2-intensity signal"
)
@click.option(
    "--period", default="hour", type=click.Choice(["5minute", "hour", "day", "week", "month"])
)
@click.pass_context
def energy_fossil_consumption(
    ctx, start_time, end_time, energy_statistic_ids, co2_statistic_id, period
):
    """Compute fossil-fuel energy consumption with the HA 2024+ signature."""
    emit(
        ctx,
        energy_advanced_core.fossil_energy_consumption(
            make_client(ctx),
            start_time=start_time,
            end_time=end_time,
            energy_statistic_ids=list(energy_statistic_ids),
...
@click.option("--energy-sources", default=None, help="JSON list of energy-source dicts (required)")
@click.option("--device-consumption", default=None, help="JSON list of device-consumption dicts")
@click.option(
    "--manual-statistic-id",
    "manual_statistic_ids",
    multiple=True,
...
@click.pass_context
def energy_save_prefs_structured(
    ctx, energy_sources, device_consumption, manual_statistic_ids, from_file
):
...
@click.option(
    "--adjustment", type=float, required=True, help="Signed float to add to the stored sum"
)
@click.option(
    "--adjustment-unit",
...
@click.pass_context
def statistics_adjust_sum(
    ctx, statistic_id, start_time, adjustment, adjustment_unit_of_measurement
):
    """Adjust the stored sum of a statistic series from a point in time."""
    emit(
        ctx,
        statistics_admin_core.adjust_sum_statistics(
            make_client(ctx),
            statistic_id=statistic_id,
            start_time=start_time,
...
@click.pass_context
def statistics_change_unit(ctx, statistic_id, new_unit_of_measurement, old_unit_of_measurement):
    """Convert all stored statistics for a series to a new unit."""
    emit(
        ctx,
        statistics_admin_core.change_statistics_unit(
            make_client(ctx),
            statistic_id=statistic_id,
...
@click.argument("statistic_id")
@click.option("--issue-type", required=True, help="e.g. unsupported_state_class, units_changed")
...
def statistics_update_issue(ctx, statistic_id, issue_type):
    ...
    emit(
        ctx,
        statistics_admin_core.update_statistics_issues(
            make_client(ctx),
            statistic_id=statistic_id,
...
@click.argument("statistic_id")
@click.option(
    "--unit", "unit_of_measurement", default=None, help="New stored unit. Pass empty/omit to clear."
)
...
def statistics_update_stored_metadata(ctx, statistic_id, unit_of_measurement):
    ...
    emit(
        ctx,
        statistics_admin_core.update_statistics_metadata(
            make_client(ctx),
            statistic_id=statistic_id,
            unit_of_measurement=unit_of_measurement,
        ),
...
@click.option("--metadata", "metadata_json", default=None, help="Inline JSON for the metadata dict")
@click.option("--stats", "stats_json", default=None, help="Inline JSON for the stats list")
@click.option(
    "--from-file",
    "from_file",
    type=click.Path(exists=True, dir_okay=False),
...
def statistics_import(ctx, metadata_json, stats_json, from_file):
    ...
    emit(
        ctx,
        statistics_admin_core.import_statistics(
            make_client(ctx),
            metadata=metadata,
            stats=stats,
        ),
...
@click.option(
    "--flow-type", default="config_flow", type=click.Choice(["config_flow", "options_flow"])
)
@click.option(
    "--user-input", "user_input_json", default=None, help="JSON dict of user input (required)"
)
@click.option(
    "--from-file",
    "from_file",
...
@click.pass_context
def helper_preview_start(ctx, domain, flow_id, flow_type, user_input_json, from_file, max_events):
...
@click.pass_context
def history_ext_with_stats_fallback(ctx, entity_id, start_iso, end_iso, period, value_field):
    ...
    emit(
        ctx,
        history_ext_core.history_with_stats_fallback(
            make_client(ctx),
            entity_id=entity_id,
            start=start,
            end=end,
...
def history_ext_retention_estimate(ctx, entity_id, probe_days):
    ...
    emit(
        ctx,
        history_ext_core.recorder_retention_estimate(
            make_client(ctx),
            entity_id=entity_id,
...
@click.argument("statistic_id")
@click.option("--stats-json", default=None, help="Inline JSON for the stats_response dict")
@click.option(
    "--from-file", "from_file", type=click.Path(exists=True, dir_okay=False), default=None
)
@click.option(
...
@click.pass_context
def history_ext_stats_to_samples(ctx, statistic_id, stats_json, from_file, value_field):
    ...
    emit(
        ctx,
        history_ext_core.statistics_to_samples(
            stats_resp,
            statistic_id=statistic_id,
            value_field=value_field,
        ),
...
@click.option("--entity", "entity_ids", multiple=True, required=True, help="entity_id (repeatable)")
@click.option("--minimal-response", is_flag=True, default=False)
@click.option("--no-attributes", is_flag=True, default=False)
@click.option("--significant-only/--all-changes", "significant_changes_only", default=True)
@click.pass_context
def history_logbook_during_period(
    ctx, start_time, end_time, entity_ids, minimal_response, no_attributes, significant_changes_only
):
    ...
    emit(
        ctx,
        history_logbook_core.history_during_period(
            make_client(ctx),
            start_time=start_time,
            end_time=end_time,
            entity_ids=list(entity_ids),
...
@click.option("--start", "start_time", required=True)
@click.option("--end", "end_time", default=None)
@click.option("--entity", "entity_ids", multiple=True, help="Filter by entity_id (repeatable)")
@click.option("--device", "device_ids", multiple=True, help="Filter by device_id (repeatable)")
@click.option("--context-id", default=None)
@click.pass_context
def history_logbook_events(ctx, start_time, end_time, entity_ids, device_ids, context_id):
    """Fetch logbook events for a period (WS logbook/get_events)."""
    emit(
        ctx,
        history_logbook_core.logbook_get_events(
            make_client(ctx),
            start_time=start_time,
            end_time=end_time,
...
def history_logbook_stream_cmd(ctx, start_time, end_time, entity_ids, device_ids):
    ...
    emit(
        ctx,
        history_logbook_core.logbook_event_stream(
            make_client(ctx),
            start_time=start_time,
            end_time=end_time,
            entity_ids=list(entity_ids) or None,
...
@click.argument("url_path")
@click.option(
    "--rule", "rules", multiple=True, help="Restrict to a subset of rule names (repeatable)"
)
@click.option("--viewport", default="mobile", type=click.Choice(["mobile", "desktop", "both"]))
@click.option(
    "--summary",
    is_flag=True,
    default=False,
...
@click.option("--card", "card_json", default=None, help="Inline JSON for the inner card dict")
@click.option(
    "--from-file",
    "from_file",
    type=click.Path(exists=True, dir_okay=False),
...
@click.option(
    "--heading-style", default="title", type=click.Choice(["title", "subtitle", "default"])
)
@click.option("--top-margin/--no-top-margin", default=False)
@click.pass_context
def lovelace_section_hero(ctx, card_json, from_file, column_span, heading_style, top_margin):
    ...
    emit(
        ctx,
        lovelace_sections_ext_core.hero_section(
            card=card,
            column_span=column_span,
...
def lovelace_section_divider(ctx, label, column_span):
    ...
    emit(
        ctx,
        lovelace_sections_ext_core.divider_section(
            label=label,
            column_span=column_span,
        ),
    )
...
@click.option("--section-json", default=None, help="Inline JSON for the section dict to mutate")
@click.option(
    "--from-file", "from_file", type=click.Path(exists=True, dir_okay=False), default=None
)
@click.option("--heading-style", default=None, type=click.Choice(["title", "subtitle", "default"]))
...
@click.pass_context
def lovelace_section_with_options(
    ctx, section_json, from_file, heading_style, top_margin, column_span, row_span
):
    ...
    emit(
        ctx,
        lovelace_sections_ext_core.with_section_options(
            section,
            heading_style=heading_style,
            top_margin=top_margin,
            column_span=column_span,
...
@click.option("--sections-file", type=click.Path(exists=True, dir_okay=False), default=None)
@click.option("--max-columns", type=int, default=4, show_default=True)
@click.option("--dense-section-placement/--no-dense", "dense_section_placement", default=None)
@click.option("--top-margin/--no-top-margin", "top_margin", default=None)
@click.option("--icon", default=None)
...
@click.pass_context
def lovelace_view_build_sections(
    ctx,
    title,
    path,
    sections,
    ...
    emit(
        ctx,
        lovelace_views_core.view_sections(
            title=title,
            path=path,
            sections=sec_list,
            max_columns=max_columns,
            dense_section_placement=dense_section_placement,
            top_margin=top_margin,
            icon=icon,
            theme=theme,
            subview=subview,
...
@click.option("--cards-file", type=click.Path(exists=True, dir_okay=False), default=None)
@click.option("--icon", default=None)
@click.option("--theme", default=None)
@click.option("--subview", is_flag=True, default=False)
@click.option("--back-path", default=None)
@click.pass_context
def lovelace_view_build_masonry(
    ctx, title, path, cards, cards_file, icon, theme, subview, back_path
):
    ...
    emit(
        ctx,
        lovelace_views_core.view_masonry(
            title=title,
            path=path,
            cards=card_list,
            icon=icon,
...
@click.option("--card-file", type=click.Path(exists=True, dir_okay=False), default=None)
@click.option("--icon", default=None)
@click.option("--theme", default=None)
@click.option("--subview", is_flag=True, default=False)
@click.option("--back-path", default=None)
@click.pass_context
def lovelace_view_build_panel(
    ctx, title, path, card_json, card_file, icon, theme, subview, back_path
):
    ...
    emit(
        ctx,
        lovelace_views_core.view_panel(
            title=title,
            card=card,
            path=path,
            icon=icon,
...
@click.option("--back-path", default=None)
@click.pass_context
def lovelace_view_build_sidebar(
    ctx, title, path, cards, cards_file, icon, theme, subview, back_path
):
    ...
    emit(
        ctx,
        lovelace_views_core.view_sidebar(
            title=title,
            path=path,
            cards=card_list,
            icon=icon,
...
@click.option("--back-path", default=None)
@click.pass_context
def lovelace_view_build_grid_layout(
    ctx,
    title,
    path,
    cards,
    ...
    emit(
        ctx,
        lovelace_views_core.view_grid_layout(
            title=title,
            path=path,
            cards=card_list,
...
@click.option("--back-path", default=None)
@click.pass_context
def lovelace_view_build_masonry_layout(
    ctx,
    title,
    path,
    cards,
    ...
    emit(
        ctx,
        lovelace_views_core.view_masonry_layout(
            title=title,
            path=path,
            cards=card_list,
            width=width,
...
@click.argument("view_path")
@click.option(
    "--conditions", default=None, help="JSON for visibility conditions, `true`, `false`, or `null`"
)
@click.option("--dry-run", is_flag=True, default=False)
...
@click.option("--event-type", default=None, help="Filter to this event type (omit for all events)")
...
@click.option(
    "--entity", "entity_ids", multiple=True, help="Filter to these entity_ids (repeatable)"
)
@click.option("--max-events", type=int, default=10, show_default=True)
@click.pass_context
...
@click.option("--trigger", "trigger_json", default=None, help="Inline JSON for the trigger dict")
@click.option(
    "--from-file",
    "from_file",
    type=click.Path(exists=True, dir_okay=False),
...
def state_stream_collect(ctx, event_type, count, timeout_seconds):
    ...
    emit(
        ctx,
        state_stream_core.collect_events(
            make_client(ctx),
            event_type=event_type,
            count=count,
...
@click.option("--domain", default=None, type=click.Choice(["automation", "script"]))
...
@click.option("--domain", required=True, type=click.Choice(["automation", "script"]))
...
def trace_debug_contexts(ctx, domain, item_id):
    ...
    emit(
        ctx,
        trace_debug_core.list_contexts(
            make_client(ctx),
            domain=domain,
            item_id=item_id,
        ),
...
@click.option("--domain", required=True, type=click.Choice(["automation", "script"]))
@click.option("--item-id", required=True)
@click.option("--node", required=True, help='Trace-node path, e.g. "action/0"')
@click.option("--run-id", default=None, help="Restrict the breakpoint to a specific run")
...
def trace_debugger_set_breakpoint(ctx, domain, item_id, node, run_id):
    ...
    emit(
        ctx,
        trace_debugger_core.set_breakpoint(
            make_client(ctx),
            domain=domain,
            item_id=item_id,
            node=node,
...
def trace_debugger_clear_breakpoint(ctx, domain, item_id, node, run_id):
    ...
    emit(
        ctx,
        trace_debugger_core.clear_breakpoint(
            make_client(ctx),
            domain=domain,
            item_id=item_id,
            node=node,
...
def trace_debugger_step(ctx, domain, item_id, run_id):
    ...
    emit(
        ctx,
        trace_debugger_core.step_execution(
            make_client(ctx),
            domain=domain,
            item_id=item_id,
            run_id=run_id,
...
def trace_debugger_continue(ctx, domain, item_id, run_id):
    ...
    emit(
        ctx,
        trace_debugger_core.continue_execution(
            make_client(ctx),
            domain=domain,
            item_id=item_id,
            run_id=run_id,
...
def trace_debugger_stop(ctx, domain, item_id, run_id):
    ...
    emit(
        ctx,
        trace_debugger_core.stop_execution(
            make_client(ctx),
            domain=domain,
            item_id=item_id,
            run_id=run_id,

...
| **New in v1.56 — Matter** | |
| `matter`           | The Matter integration's websocket API — commissioning and node operations. `available` (a READ: 'not loaded' is an answer, exit 0), `nodes` (device registry with `node_id` / `fabric_id` / `endpoint` decoded from HA's hex identifiers), `commission <code>` (QR payload or manual code; `--no-network-only` lets the device change networks), `commission-on-network <PIN>` (`--ip` skips discovery; pin is the 8- or 11-digit setup PIN as an integer), `set-wifi <ssid>` / `set-thread <dataset>` (credentials for LATER commissioning — nothing already on the network moves; the Wi-Fi password is prompted hidden), `node-diagnostics` / `ping` / `interview` / `open-commissioning-window` / `remove-fabric <device> <index 1..254>` (confirmation-gated). Node-scoped commands take a device id OR any entity id on the device. A successful commission answers an EMPTY result — read `matter nodes` to see what joined. Commissioning can take minutes: raise `--timeout`. |

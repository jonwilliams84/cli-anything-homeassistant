...
def grid(
    cards: list[dict], *, columns: int = 2, square: bool = False, title: str | None = None
) -> dict:
    if not cards:
        raise ValueError("at least one card required")
    if columns <= 0:
...
def glance(
    ...
    *,
    ...
    if show_state is not None:
        card["show_state"] = show_state
    if show_icon is not None:
        card["show_icon"] = show_icon
...
def markdown(content: str, *, title: str | None = None, theme: str | None = None) -> dict:
    if not content:
...
def history_graph(
    ...
    *,
    ...
    if refresh_interval is not None:
        card["refresh_interval"] = refresh_interval
    return card
...
def picture_elements(
    image: str, elements: list[dict], *, title: str | None = None, state_filter: dict | None = None
) -> dict:
    if not image:
    ...
    if state_filter is not None:
        card["state_filter"] = state_filter
    return card


def iframe(url: str, *, title: str | None = None, aspect_ratio: str | None = None) -> dict:
...
def weather_forecast(
    ...
    *,
    ...
    if not show_forecast:
        card["show_forecast"] = False
    if forecast_type != "daily":
        card["forecast_type"] = forecast_type
    if theme is not None:
        card["theme"] = theme
    return card
...
def mushroom_template(
    ...
    *,
    ...
    multiline_secondary: bool = False,
) -> dict:
    ...
    if badge_icon is not None:
        card["badge_icon"] = badge_icon
    if badge_color is not None:
        card["badge_color"] = badge_color
    ...
    if fill_container:
        card["fill_container"] = True
    if multiline_secondary:
...
def mushroom_light(
    ...
    *,
    ...
    if icon is not None:
        card["icon"] = icon
    if show_brightness_control:
        card["show_brightness_control"] = True
    if show_color_control:
        card["show_color_control"] = True
    if show_color_temp_control:
        card["show_color_temp_control"] = True
    if not use_light_color:
        card["use_light_color"] = False
    if collapsible_controls:
        card["collapsible_controls"] = True
...
def mushroom_climate(
    ...
    *,
    ...
    if icon is not None:
        card["icon"] = icon
    if show_temperature_control:
        card["show_temperature_control"] = True
    if hvac_modes is not None:
        card["hvac_modes"] = hvac_modes
    if collapsible_controls:
        card["collapsible_controls"] = True
    return card
...
def mushroom_title(
    title: str | None = None, subtitle: str | None = None, alignment: str | None = None
) -> dict:
...
def apexcharts(
    ...
    *,
    ...
    if graph_span is not None:
        card["graph_span"] = graph_span
    if chart_type is not None:
        card["chart_type"] = chart_type
    if stacked is not None:
        card["stacked"] = stacked
    if apex_config is not None:
        card["apex_config"] = apex_config
    if yaxis is not None:
        card["yaxis"] = yaxis
    if all_yaxis is not None:
        card["all_yaxis"] = all_yaxis
    return card
...
def mini_graph(
    ...
    *,
    ...
    if points_per_hour is not None:
        card["points_per_hour"] = points_per_hour
    if line_width is not None:
        card["line_width"] = line_width
...
def layout_card(
    cards: list[dict], *, layout_type: str = "grid", layout: dict | None = None
) -> dict:
...
def stack_in_card(
    cards: list[dict], *, mode: str = "vertical", title: str | None = None, keep: dict | None = None
) -> dict:
...
def simple_weather(
    ...
    *,
    ...
    if primary_info is not None:
        card["primary_info"] = primary_info
    if secondary_info is not None:
        card["secondary_info"] = secondary_info
...
def atomic_calendar(
    ...
    *,
    ...
    show_no_event_days: bool | None = None,
) -> dict:
    ...
    if default_mode not in ("Event", "EventList", "Calendar"):
        raise ValueError(f"default_mode must be Event / EventList / Calendar, got {default_mode!r}")
    norm: list[dict] = []
    for e in entities:
        if isinstance(e, str):
    ...
    if show_location is not None:
        card["showLocation"] = show_location
    if show_no_event_days is not None:
        card["showNoEventDays"] = show_no_event_days
    return card
...
def digital_clock(
    *,
    ...
    if time_format is not None:
        card["timeFormat"] = time_format
    if date_format is not None:
        card["dateFormat"] = date_format
    if locale is not None:
        card["locale"] = locale
    if time_zone is not None:
        card["timeZone"] = time_zone
    return card
...
def flex_table(
    *, entities, columns: list[dict], sort_by: str | None = None, strict: bool | None = None
) -> dict:
    ...
    *,
    ...
    for e in entities:
        if isinstance(e, str):
            ...
            raise ValueError(f"entities[*] must be str or dict, got {type(e).__name__}")
    ...
    for i, ts in enumerate(toolsets):
        ...
        for j, t in enumerate(ts["tools"]):
            ...
            if tt not in SAK_TOOL_TYPES:
                raise ValueError(
                    f"toolsets[{i}].tools[{j}] has unknown type {tt!r}; valid: {SAK_TOOL_TYPES}"
                )
...
def ensure_sak_templates_on_dashboard(
    client,
    url_path: str | None,
    *,
...
def fetch_sak_system_templates(*, branch: str = "master") -> dict:
    ...
    try:
        ...
        raise RuntimeError("PyYAML required to load SAK templates (pip install pyyaml)") from e

    base = "https://api.github.com"
    repo = "AmoebeLabs/swiss-army-knife-card"
    root_path = "ha-config/lovelace/sak_templates/templates"

    def _require_https(u: str) -> None:
        """Reject any URL that is not plain HTTPS before it reaches urlopen.

        urllib honours file:, ftp: and other schemes, so a urlopen() on a URL
        assembled from caller-supplied values (branch/path here, and
        `download_url` taken straight from the API response) can be steered at
        the local filesystem (bandit B310). GitHub only ever serves these over
        HTTPS, so anything else is a bug or an attack and should not be fetched.
        """
        if not u.startswith("https://"):
            raise RuntimeError(f"refusing to fetch non-HTTPS URL: {u!r}")

    def _gh(path):
        url = f"{base}/repos/{repo}/contents/{path}?ref={branch}"
        _require_https(url)
        with urllib.request.urlopen(url) as r:  # nosec B310 # noqa: S310 - _require_https above
    ...
    for entry in dirs:
        ...
        for f in _gh(f"{root_path}/{name}"):
            ...
            with urllib.request.urlopen(download_url) as r:  # nosec B310 # noqa: S310 - _require_https above
...
def sak_toolset(
    name: str, *, cx: float = 50, cy: float = 50, tools: list[dict] | None = None
) -> dict:
    return {
        "toolset": name,
...
def sak_ellipse(
    *, cx: float, cy: float, rx: float, ry: float, styles: dict | None = None, **kw
) -> dict:
    return _sak_tool(
        "ellipse", position={"cx": cx, "cy": cy, "rx": rx, "ry": ry}, styles=styles, **kw
    )


def sak_line(
    *, x1: float, y1: float, x2: float, y2: float, styles: dict | None = None, **kw
) -> dict:
    return _sak_tool("line", position={"x1": x1, "y1": y1, "x2": x2, "y2": y2}, styles=styles, **kw)
...
def sak_rectangle(
    ...
    **kw,
    ...
    pos: dict[str, Any] = {"cx": cx, "cy": cy, "width": width, "height": height}
    if rx is not None:
        pos["rx"] = rx
...
def sak_text(
    ...
    **kw,
    ...
    if align is not None:
        pos["align"] = align
    return _sak_tool(
        "text", text=text, position=pos, entity_index=entity_index, styles=styles, **kw
    )
...
def sak_icon(
    ...
    **kw,
    ...
    return _sak_tool(
        "icon",
        position={"cx": cx, "cy": cy, "align": align, "icon_size": icon_size},
        entity_index=entity_index,
        styles=styles,
        **kw,
    )
...
def sak_state(
    *, cx: float, cy: float, entity_index: int = 0, styles: dict | None = None, **kw
) -> dict:
    return _sak_tool(
        "state", position={"cx": cx, "cy": cy}, entity_index=entity_index, styles=styles, **kw
    )
...
def sak_name(
    *, cx: float, cy: float, entity_index: int = 0, styles: dict | None = None, **kw
    ...
    return _sak_tool(
        "name", position={"cx": cx, "cy": cy}, entity_index=entity_index, styles=styles, **kw
    )
...
def sak_horseshoe(
    *, cx: float, cy: float, radius: float, entity_index: int = 0, styles: dict | None = None, **kw
) -> dict:
    return _sak_tool(
        "horseshoe",
        position={"cx": cx, "cy": cy, "radius": radius},
...
def sak_sparkline(
    ...
    **kw,
    ...
    return _sak_tool(
        "sparkline",
        position={"cx": cx, "cy": cy, "width": width, "height": height, "hours": hours},
        entity_index=entity_index,
        styles=styles,
        **kw,
    )
...
def sak_slider(
    ...
    **kw,
    ...
    return _sak_tool(
        "slider",
        position={"cx": cx, "cy": cy, "length": length, "orientation": orientation},
        entity_index=entity_index,
        styles=styles,
        **kw,
    )
...
def sak_switch(
    ...
    **kw,
    ...
    return _sak_tool(
        "switch",
        position={"cx": cx, "cy": cy, "width": width, "height": height},
        entity_index=entity_index,
        styles=styles,
        **kw,
    )
...
def sak_usersvg(
    *, cx: float, cy: float, width: float, height: float, uri: str, styles: dict | None = None, **kw
) -> dict:
...
def sak_regpoly(
    *, cx: float, cy: float, radius: float, sides: int = 6, styles: dict | None = None, **kw
) -> dict:
    return _sak_tool(
        "regpoly",
        position={"cx": cx, "cy": cy, "radius": radius, "sides": sides},
        styles=styles,
        **kw,
    )
...
def sak_rectex(
    *, cx: float, cy: float, width: float, height: float, styles: dict | None = None, **kw
) -> dict:
...
def sak_area(
    *, cx: float, cy: float, entity_index: int = 0, styles: dict | None = None, **kw
    ...
    return _sak_tool(
        "area", position={"cx": cx, "cy": cy}, entity_index=entity_index, styles=styles, **kw
    )
...
def modern_circular_gauge(
    ...
    *,
    ...
    if show_graph is not None:
        card["show_graph"] = show_graph
    if adaptive_icon_color is not None:
        card["adaptive_icon_color"] = adaptive_icon_color
    if secondary is not None:
        card["secondary"] = secondary
...
def horizon_card(
    *,
    ...
    if refresh_period is not None:
        card["refresh_period"] = refresh_period
    if southern_flip:
        card["southern_flip"] = True
    if fields is not None:
        card["fields"] = fields
    if language is not None:
        card["language"] = language
    if time_format is not None:
        card["time_format"] = time_format
    if no_card:
        card["no_card"] = True
    return card
...
def calendar_card_pro(
    ...
    *,
    ...
    if show_location is not None:
        card["show_location"] = show_location
    if title is not None:
        card["title"] = title
    if max_events is not None:
        card["max_events"] = max_events
    if compact_mode is not None:
        card["compact_mode"] = compact_mode
    if time_format is not None:
        card["time_format"] = time_format
    return card
...
def room_summary_card(
    ...
    *,
    ...
    problem_entities: list[str] | None = None,
) -> dict:
    ...
    if features is not None:
        ...
        if unknown:
            raise ValueError(
                f"unknown feature flags {unknown}; valid: {sorted(_ROOM_SUMMARY_FEATURES)}"
            )
    ...
    if background is not None:
        card["background"] = background
    if sensor_classes is not None:
        card["sensor_classes"] = sensor_classes
    if problem_entities is not None:
        card["problem_entities"] = problem_entities
    return card
...
def expander_card(
    ...
    *,
    ...
    expander_card_id: str | None = None,
) -> dict:
    ...
    if clear:
        card["clear"] = True
    if clear_children:
        card["clear-children"] = True
    if gap is not None:
        card["gap"] = gap
    if padding is not None:
        card["padding"] = padding
    if title_card is not None:
        card["title-card"] = title_card
    if title_card_clickable:
        card["title-card-clickable"] = True
    if storage_id is not None:
        card["storage-id"] = storage_id
    if expander_card_id is not None:
        card["expander-card-id"] = expander_card_id
    return card
...
def simple_swipe_card(
    ...
    *,
    ...
    if show_pagination is not None:
        card["show_pagination"] = show_pagination
    if card_spacing is not None:
        card["card_spacing"] = card_spacing
    if enable_auto_swipe:
        card["enable_auto_swipe"] = True
    if auto_swipe_interval is not None:
        card["auto_swipe_interval"] = auto_swipe_interval
    if state_entity is not None:
        card["state_entity"] = state_entity
    return card
...
def build(card_type: str, **kwargs) -> dict:
    ...
    if card_type not in BUILDERS:
        raise ValueError(f"unknown card type {card_type!r}; known: {', '.join(list_builders())}")
...
BUILDER_META: dict[str, dict] = {
    ...
    "glance": {
        ...
        "example": {
            "type": "glance",
            "title": "Status",
            "entities": ["sensor.outdoor_temp", "sensor.indoor_temp"],
        },
    },
    "gauge": {
        "card_type": "gauge",
    ...
    "markdown": {
        ...
        "example": {"type": "markdown", "title": "Hello", "content": "## Welcome, {{ user }}"},
    },
    "history-graph": {
    ...
    "conditional": {
        ...
        "example": {
            "type": "conditional",
            "conditions": [{"entity": "binary_sensor.guest_mode", "state": "on"}],
    ...
    "apexcharts": {
        ...
        "example": {
            ...
            "series": [
                {"entity": "sensor.smart_meter_power", "name": "Grid", "type": "area"},
            ],
        },
    },
    "mini-graph": {
        ...
        "example": {
            "type": "custom:mini-graph-card",
            "entities": [{"entity": "sensor.outdoor_temp", "name": "Outdoor"}],
            "hours_to_show": 24,
            "line_width": 3,
    ...
    "calendar-card-pro": {
        ...
        "example": {
            "type": "custom:calendar-card-pro",
            "entities": [{"entity": "calendar.personal", "color": "#FF7A00"}],
            "days_to_show": 7,
        },
    },
    "decluttering": {
    ...
    "horizon": {
        ...
        "example": {
            ...
            "title": "Sun",
            "fields": {"sunrise": True, "sunset": True, "dawn": True, "dusk": True},
        },
    },
    "modern-circular-gauge": {
    ...
    "room-summary": {
        ...
        "example": {"type": "custom:room-summary-card", "area": "kitchen"},
    },
    "simple-swipe": {
    ...
    "atomic-calendar": {
        ...
        "example": {
            "type": "custom:atomic-calendar-revive",
            "entities": [{"entity": "calendar.personal", "name": "Personal", "color": "#FF7A00"}],
            "defaultMode": "Event",
            "maxDaysToShow": 14,
        },
    },
    "digital-clock": {
        ...
        "example": {
            ...
            "date_format": {"weekday": "long", "day": "numeric", "month": "long"},
        },
    },
    "flex-table": {
...
SAK_TOOL_META: dict[str, dict] = {
    ...
    "sak_line": {
        "summary": "SVG straight line.",
        "example": {"type": "line", "position": {"x1": 0, "y1": 50, "x2": 100, "y2": 50}},
    },
    "sak_rectangle": {
        "summary": "SVG rectangle.",
        "example": {
            "type": "rectangle",
            "position": {"cx": 50, "cy": 50, "width": 80, "height": 30},
        },
    },
    "sak_text": {
        "summary": "SVG text label.",
        "example": {"type": "text", "text": "ON", "position": {"cx": 50, "cy": 50}},
    },
    "sak_icon": {
        "summary": "Icon for one of the card's entities.",
        "example": {"type": "icon", "position": {"cx": 50, "cy": 30, "entity_index": 0}},
    },
    "sak_state": {
        "summary": "Live state text for an entity.",
        "example": {"type": "state", "position": {"cx": 50, "cy": 60, "entity_index": 0}},
    },
    "sak_name": {
        "summary": "Friendly name text for an entity.",
        "example": {"type": "name", "position": {"cx": 50, "cy": 80, "entity_index": 0}},
    },
    "sak_segarc": {
        "summary": "Segmented arc gauge.",
        "example": {
            "type": "segarc",
            "position": {"cx": 50, "cy": 50, "radius": 45, "start_angle": 130, "end_angle": 410},
        },
    },
    "sak_horseshoe": {
        "summary": "Horseshoe gauge.",
        "example": {"type": "horseshoe", "position": {"cx": 50, "cy": 50, "radius": 45}},
    },
    "sak_sparkline": {
        "summary": "Mini sparkline graph.",
        "example": {
            "type": "sparkline",
            "position": {"cx": 50, "cy": 50, "width": 80, "height": 30},
            "hours": 24,
    ...
    "sak_switch": {
        "summary": "Toggle switch shape.",
        "example": {"type": "switch", "position": {"cx": 50, "cy": 50}},
    },
    "sak_usersvg": {
        "summary": "Inline user-supplied SVG.",
        "example": {
            "type": "usersvg",
            "position": {"cx": 50, "cy": 50, "width": 50, "height": 50},
            "svg": "<svg>...</svg>",
        },
    },
    "sak_circslider": {
        "summary": "Circular slider.",
        "example": {"type": "circslider", "position": {"cx": 50, "cy": 50, "radius": 40}},
    },
    "sak_progpath": {
        "summary": "Progress-along-path tool.",
        "example": {
            "type": "progpath",
            "position": {"cx": 50, "cy": 50, "width": 80, "height": 30},
            "path": "M0,0 L100,100",
    ...
    "sak_regpoly": {
        "summary": "Regular polygon (hex, octagon, etc.).",
        "example": {"type": "regpoly", "position": {"cx": 50, "cy": 50, "radius": 40, "sides": 6}},
    },
    "sak_rectex": {
        "summary": "Extended rectangle with rounded corners.",
        "example": {
            "type": "rectex",
            "position": {"cx": 50, "cy": 50, "width": 80, "height": 30, "rx": 5, "ry": 5},
        },
    },
    "sak_area": {
        "summary": "Area chart filled below sparkline.",
        "example": {
            "type": "area",
            "position": {"cx": 50, "cy": 50, "width": 80, "height": 30, "entity_index": 0},
            "hours": 24,
        },
    },
    "sak_toolset": {
        "summary": "Toolset container — groups tools at a position.",
...
def generate_cards_reference() -> str:
    ...
    """
    native = [n for n in list_builders() if BUILDER_META.get(n, {}).get("resource") is None]
    custom = [n for n in list_builders() if BUILDER_META.get(n, {}).get("resource") is not None]
    ...
    lines.append("")
    lines.append(
        "These are not standalone cards — they are tools you nest "

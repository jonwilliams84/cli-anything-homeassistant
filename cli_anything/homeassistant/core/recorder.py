...
def entity_stats(client, entity_id: str, *, hours: float = 24) -> dict:
    ...
    return {
        ...
        "live_last_changed": live.get("last_changed") if isinstance(live, dict) else None,
...
def batch_stats(client, entity_ids: Iterable[str], *, hours: float = 24) -> list[dict]:
    """Run `entity_stats` over a list and return the results."""
    return [entity_stats(client, e, hours=hours) for e in entity_ids]


def find_unrecorded(client, entity_ids: Iterable[str], *, hours: float = 24) -> list[str]:
    ...
    return [
        s["entity_id"]
        for s in batch_stats(client, entity_ids, hours=hours)
        if s["history_points_24h"] == 0
    ]


def purge(
    client, *, keep_days: int | None = None, repack: bool = False, apply_filter: bool = False
) -> dict:
    ...
    if repack:
        data["repack"] = True
    if apply_filter:
        data["apply_filter"] = True
    return services_core.call_service(client, "recorder", "purge", service_data=data or None)
...
def purge_entities(
    ...
    *,
    ...
    data: dict = {}
    if entity_ids:
        data["entity_id"] = list(entity_ids)
    if domains:
        data["domains"] = list(domains)
    if entity_globs:
        data["entity_globs"] = list(entity_globs)
    if days is not None:
        data["days"] = days
    if not data:
        raise ValueError("provide at least one of entity_ids / domains / entity_globs")
    return services_core.call_service(client, "recorder", "purge_entities", service_data=data)
...
def top_entities(
    ...
    *,
    ...
    if entity_ids is None:
        ...
        names = {s["entity_id"]: s.get("attributes", {}).get("friendly_name") for s in states}
    else:
        ids = list(entity_ids)
        names = {}
    if domains:

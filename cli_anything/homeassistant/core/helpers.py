...
def _name_of(state: dict, fallback_eid: str) -> str:
    return state.get("attributes", {}).get("friendly_name") or fallback_eid.split(".", 1)[1]
...
def _split_eid(entity_id: str, expected_domain: str) -> str:
    ...
    if domain != expected_domain:
        raise ValueError(f"expected {expected_domain}.* entity_id, got {entity_id!r}")
    if not obj:
...
def input_select_set_options(client, entity_id: str, options: list[str]) -> dict:
    ...
    client.post(
        "services/input_select/set_options",
        {
            "entity_id": entity_id,
            "options": options,
        },
...
def input_select_create(
    client, name: str, options: list[str], *, icon: str | None = None, initial: str | None = None
) -> dict:
...
def input_select_sync(
    client, src_entity_id: str, dst_entity_id: str, *, fallback: str = "Auto"
) -> dict:
    ...
    if src_opts == dst_opts:
        return {"changed": False, "src_options": src_opts, "dst_state": dst.get("state")}
    ...
    dst_name = dst["attributes"].get("friendly_name") or dst_entity_id.split(".", 1)[1]
    input_select_update(client, dst_entity_id, options=src_opts, name=dst_name)
...
def input_boolean_create(
    client, name: str, *, icon: str | None = None, initial: bool | None = None
) -> dict:
    if not name:
...
def input_boolean_delete(client, entity_id: str) -> dict:
    return _ws_collection(
        ...
        "input_boolean",
        "delete",
        payload={"input_boolean_id": _split_eid(entity_id, "input_boolean")},
    )
...
def input_button_create(client, name: str, *, icon: str | None = None) -> dict:
    if not name:
        raise ValueError("name required")
    payload: dict = {"name": name}
    if icon:
...
def input_button_update(
    client, entity_id: str, *, name: str | None = None, icon: str | None = None
) -> dict:
...
def input_button_delete(client, entity_id: str) -> dict:
    return _ws_collection(
        ...
        "input_button",
        "delete",
        payload={"input_button_id": _split_eid(entity_id, "input_button")},
    )
...
def input_number_create(
    ...
    *,
    ...
    payload: dict = {"name": name, "min": min, "max": max, "step": step, "mode": mode}
...
def input_number_delete(client, entity_id: str) -> dict:
    return _ws_collection(
        ...
        "input_number",
        "delete",
        payload={"input_number_id": _split_eid(entity_id, "input_number")},
    )


def input_number_set_value(client, entity_id: str, value: float) -> dict:
    return client.post("services/input_number/set_value", {"entity_id": entity_id, "value": value})
...
def input_text_delete(client, entity_id: str) -> dict:
    return _ws_collection(
        ...
        "input_text",
        "delete",
        payload={"input_text_id": _split_eid(entity_id, "input_text")},
    )


def input_text_set_value(client, entity_id: str, value: str) -> dict:
    return client.post("services/input_text/set_value", {"entity_id": entity_id, "value": value})
...
def input_datetime_create(
    ...
    *,
    ...
    payload: dict = {"name": name, "has_date": has_date, "has_time": has_time}
    if icon is not None:
        payload["icon"] = icon
    if initial is not None:
        payload["initial"] = initial
...
def input_datetime_delete(client, entity_id: str) -> dict:
    return _ws_collection(
        ...
        "input_datetime",
        "delete",
        payload={"input_datetime_id": _split_eid(entity_id, "input_datetime")},
    )


def input_datetime_set(
    client,
    entity_id: str,
    *,
    date: str | None = None,
...
def counter_create(
    ...
    *,
    ...
    if step <= 0:
        raise ValueError("step must be positive")
    payload: dict = {"name": name, "initial": initial, "step": step, "restore": restore}
...
def counter_delete(client, entity_id: str) -> dict:
    return _ws_collection(
        client, "counter", "delete", payload={"counter_id": _split_eid(entity_id, "counter")}
    )
...
def counter_set_value(client, entity_id: str, value: int) -> dict:
    return client.post("services/counter/set_value", {"entity_id": entity_id, "value": value})
...
def timer_create(
    client, name: str, *, duration: str = "00:00:00", restore: bool = False, icon: str | None = None
) -> dict:
...
def timer_update(
    ...
    *,
    ...
    payload: dict = {
        "timer_id": obj,
        "name": name if name is not None else _name_of(state, entity_id),
        "duration": duration if duration is not None else attrs.get("duration", "00:00:00"),
        "restore": restore if restore is not None else attrs.get("restore", False),
    }
    if icon is not None:
        payload["icon"] = icon
...
def timer_delete(client, entity_id: str) -> dict:
    return _ws_collection(
        client, "timer", "delete", payload={"timer_id": _split_eid(entity_id, "timer")}
    )
...
def timer_change(client, entity_id: str, duration: str) -> dict:
    ...
    return client.post("services/timer/change", {"entity_id": entity_id, "duration": duration})
...
def schedule_delete(client, entity_id: str) -> dict:
    return _ws_collection(
        client, "schedule", "delete", payload={"schedule_id": _split_eid(entity_id, "schedule")}
    )
...
def person_create(
    ...
    *,
    ...
    if not name:
        raise ValueError("name required")
    payload: dict = {"name": name, "device_trackers": list(device_trackers or [])}
...
def person_delete(client, person_id: str) -> dict:
    if not person_id:
        raise ValueError("person_id required")
    return _ws_collection(client, "person", "delete", payload={"person_id": person_id})
...
def zone_delete(client, zone_id: str) -> dict:
    if not zone_id:
        raise ValueError("zone_id required")
    return _ws_collection(client, "zone", "delete", payload={"zone_id": zone_id})
...
def tag_create(
    client, tag_id: str, *, name: str | None = None, description: str | None = None
) -> dict:
    if not tag_id:
...
def config_entries_list(
    ...
    entries = _ws_collection(client, "config_entries", "get", payload=payload) or []

...
def filter_entities_by_domain(entities: list[dict], domain: str) -> list[dict]:
    return [e for e in entities if str(e.get("entity_id", "")).startswith(f"{domain}.")]
...
def update_entity(
    ...
    *,
    ...
    if disabled_by is not _SENTINEL:
        payload["disabled_by"] = disabled_by
...
def bulk_update_entities(client, *, updates: list[dict], dry_run: bool = False) -> dict:
    ...
    for u in updates:
        eid = u.get("entity_id")
        if not eid:
            failed.append({"entity_id": None, "error": "missing entity_id", "fields": u})
            continue

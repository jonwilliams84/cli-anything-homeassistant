...
def card_height_class(card: dict) -> int:
    ...
    if t in ("history-graph", "statistics-graph", "weather-forecast", "media-control"):
        return _TALL
    if t in (
        "light",
        "thermostat",
    ...
    if t == "horizontal-stack":
        return max((card_height_class(c) for c in (card.get("cards") or [])), default=_SHORT)
    if t == "grid":
    ...
    if t.startswith("custom:"):
        ...
        if (
            "apexcharts" in bare
            or "weather-chart" in bare
            or "atomic-calendar" in bare
            or "calendar-card-pro" in bare
            or "flex-table" in bare
            or "horizon-card" in bare
            or "weather-card" in bare
        ):
            return _HUGE
        if (
            "mini-graph" in bare
            or "mini-media-player" in bare
            or "modern-circular-gauge" in bare
            or "button-card" in bare
        ...
        if (
            "stack-in-card" in bare
            or "expander-card" in bare
            or "layout-card" in bare
            or "decluttering-card" in bare
...
def _check_heading_truncation(card: dict, path: str, column_span: int, viewport: str) -> list[dict]:
    ...
    return [
        {
            ...
            "message": (
                f"heading {width} chars exceeds {ctx} budget ({budget} "
                f"chars) — will truncate with ellipsis"
            ),
            "hint": (
                "shorten the heading, drop heading_style to default, or "
...
def _check_useless_single_child_stack(card: dict, path: str) -> list[dict]:
    ...
    if card.get("type") not in ("vertical-stack", "horizontal-stack", "custom:stack-in-card"):
        return []
    cards = card.get("cards") or []
    if len(cards) == 1:
        return [
            {
                "severity": "info",
                "path": path,
                "rule": "useless-single-stack",
                "message": (
                    f"{card['type']!r} contains only one child — adds chrome with no layout benefit"
                ),
...
def _check_horizontal_stack_squeeze(
    card: dict, path: str, column_span: int, viewport: str
) -> list[dict]:
            ...
            return [
                {
                    ...
                    "rule": "hstack-squeeze",
                    "message": (
                        f"horizontal-stack with {len(children)} children "
                        f"will collapse to a vertical pile on mobile"
                    ),
                    "hint": (
                        "on mobile-first dashboards, prefer 2-card hstacks "
        ...
        return [
            {
                ...
                "hint": (
                    f"widen the section to column_span={needed}+ OR replace with a 2-column grid"
                ),
            }
        ]
    return []
...
def _check_sibling_height_mismatch(card: dict, path: str, viewport: str) -> list[dict]:
    ...
    if card.get("type") not in ("grid", "horizontal-stack", "custom:layout-card"):
        return []
            ...
            issues.append(
                {
                    ...
                    "message": (
                        f"row of {len(row)} sibling cards has uneven "
                        f"heights (weights {weights}, types {types}) — "
                        f"will look ragged"
                    ),
                    "hint": (
                        "group small cards into a vertical-stack so each "
...
def _check_default_digital_clock(
    card: dict, path: str, column_span: int, viewport: str
) -> list[dict]:
    ...
    return [
        {
            ...
            "message": (
                f"digital-clock without font-size override will dwarf siblings on {viewport}"
            ),
            "hint": ("add card_mod with `--ha-card-header-font-size: 1.5em` or similar"),
        }
    ]
...
def _check_tight_grid_with_text_heavy(card: dict, path: str) -> list[dict]:
    ...
    heavy = [c for c in children if isinstance(c, dict) and c.get("type") in TEXT_HEAVY]
    if heavy:
        return [
            {
                "severity": "warning",
                ...
                "message": (
                    f"grid with columns={cols} contains {len(heavy)} "
                    f"text-heavy card(s) "
                    f"({[c.get('type') for c in heavy]}) — text will "
                    f"truncate in narrow columns"
                ),
                "hint": ("reduce to columns=2 or move text-heavy cards out into their own row"),
            }
        ]
    return []
...
def _walk_with_column_span(node: Any, path: str = "", column_span: int = 4):
    ...
    if isinstance(node, dict):
        ...
        if is_card and ((path.endswith("]") and "/cards[" in path) or "/card" in path):
    ...
    return [
        {
            "severity": "warning",
            "path": path,
            "rule": "panel-multi-card",
            "message": (
                f"panel view has {len(cards)} cards but panel layout "
    ...
    return [
        {
            ...
            "message": (
                "sections view without max_columns defaults to 4 on "
...
def _check_view_sidebar_deprecated(view: dict, path: str) -> list[dict]:
    if view.get("type") != "sidebar":
        return []
    return [
        {
            "severity": "info",
            "path": path,
            "rule": "sidebar-deprecated",
            "message": (
                "sidebar view layout is deprecated — still works but won't get new features"
            ),
...
def _check_subview_back_path(view: dict, path: str) -> list[dict]:
    if not view.get("subview"):
        return []
    if view.get("back_path"):
        return []
    return [
        {
            "severity": "info",
            "path": path,
            "rule": "subview-no-back-path",
            "message": (
                "subview without back_path — the back button defaults to "
...
def lint_layout(
    dash: dict, *, rules: set[str] | None = None, viewport: str = "mobile"
) -> list[dict]:
    ...
    for path, card, col_span in _walk_with_column_span(dash):
        for fn, rule_name in rule_fns:
            ...
            try:
                ...
                kwargs = {}
                if "column_span" in params:
                    kwargs["column_span"] = col_span
